...
year_data <- data %>%
  ...
  mutate(year_growth = value - lag(value, default = first(value))) %>%
...
psem_data <- year_data %>%
  ...
  select(-c(genotype:season), -highest_light, -average_light) %>%
  drop_na()

# Fit the full model 
full_model <- lm(year_growth ~., data = psem_data)
