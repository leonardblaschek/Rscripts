...
rmn_irx_long <- model_data_pop %>%
  mutate(n_v = n_v * 100) %>%
  select(genotype, replicate, technical, cell_type, n_v) %>%
...
pdf("IRX_FigureS1_pop.pdf", width = twocol, height = onecol * 1.5)
adj_TE_pop + poplar_variation +
  plot_layout(nrow = 2, heights = c(1, 4)) &
  plot_annotation(tag_levels = 'A') & 
  theme(plot.tag = element_text(size = 10, face = "bold", family = "Helvetica"))
dev.off()
...
pdf("individual_intercept.pdf", width = twocol, height = twocol)
dist_lig1 + adj_TE_pop + poplar_variation +
  plot_layout(nrow = 3, heights = c(0.35, 0.2, 1)) &
  plot_annotation(tag_levels = 'A') & 
  theme(plot.tag = element_text(size = 10, face = "bold", family = "Helvetica"))
dev.off()
