...
conditions <- read_csv(
  ...
  fill(everything()) %>%
  group_by(genotype) %>%
  mutate(genotype = ordered(recode(genotype, "CAD" = "cad4x5"),
      levels = c("WT", "cad4x5")
    ),
    time = row_number()) 

transp_cond <- transpiration %>%
  left_join(conditions)

transp <- ggplot(transp_cond,
                 aes(x = time,
                     y = transp_rate,
                     colour = genotype)) +
...
pdf("transpiration_supplement.pdf", width = twocol, height = onecol)
transp
dev.off()
