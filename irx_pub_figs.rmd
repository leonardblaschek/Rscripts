...
psem_data_recent <-  model_data_pop %>%
  filter(cell_type == "SV" & bin == "recent") %>%
  ungroup() %>%
  select(
    Solidity,
    Distance,
...
pdf("interactions.pdf", width = onecol, height = twocol)
PX1 + PX2 + PX3 + PX4 + PMX1 + PMX2 + SMX1 + SMX2 + SMX3 + 
...
best_model <- find_interactions("early")
...
best_model <- find_interactions("recent")
