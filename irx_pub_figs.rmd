...
```{r, fig.showtext = TRUE, out.width = '100%', message = FALSE, eval = TRUE, echo = FALSE}
rosettes <- read_csv("/home/leonard/Documents/Uni/PhD/IRX/Drought/drought_area.csv") %>%
  mutate(
...
transpiration <- read_csv(
  "/home/leonard/Documents/Uni/PhD/IRX/Drought/evapotranspiration.csv"
) %>%
...
```{r, fig.showtext = TRUE, out.width = '100%', message = FALSE, eval = TRUE, echo = FALSE}
recovery_data <- read_csv("/home/leonard/Documents/Uni/PhD/IRX/Drought/recovery.csv") %>%
  mutate(
...
rmn_pop_sum <- rmn_pop_scaled %>%
  ...
  summarise(
    ...
    "cellulose" = 
      scaled[abs(wavenumber - 1099) == min(abs(wavenumber - 1099))][1] +
...
pop_irx <- read_csv(
  "/home/leonard/Documents/Uni/PhD/IRX/Poplar/2020-08_soil_poplar/pop_rmn_coords.csv"
  )
...
celltypes <- read_csv(
  "/home/leonard/Documents/Uni/PhD/IRX/Poplar/2020-08_soil_poplar/poplar_cell_types.csv",
  col_types = "ccccc")
...
regressions <- rmn_lignin_dist %>%
  ...
  mutate(
    ...
    value = case_when(
      genotype == "T89" ~ max(value),
      TRUE ~ max(value) - (0.1 * (max(value) - min))
    )
  )
...
p_values <- rmn_pop_data %>%
  group_by(cell_type) %>%
  summarise(p_value = broom::tidy(
      t.test(Solidity[genotype == "T89"], Solidity[genotype == "C4H"])
    )[["p.value"]]) %>%
  mutate(
...
conditions <- read_csv(
  "/home/leonard/Documents/Uni/PhD/IRX/Drought/chamber_conditions.csv"
) %>%
  fill(everything()) %>%
  group_by(genotype) %>%
  mutate(
