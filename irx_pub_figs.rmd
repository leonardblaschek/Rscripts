...
eds_plot <- function(num, den, lbl) {
  ...
  stat_tab <- letter_groups(letter_data,
    ratio,
    ID,
    "tukey",
    table = TRUE
  ...
  write_csv(anova_tab, paste0("1C_anova_", quo_name(num), "-", quo_name(den), ".csv"))
  
  tukey_tab <- stat_tab$tukey %>% distinct()
  write_csv(tukey_tab, paste0("1C_tukey_", quo_name(num), "-", quo_name(den), ".csv"))
...
stat_tab <- eds_data_supp %>%
  ...
  anova_tab <- stat_tab$anova %>% add_column("panel" = stat_tab$elements) %>% distinct()
  write_csv(anova_tab, paste0("S1_anova.csv"))

  tukey_tab <- stat_tab$tukey %>% add_column("panel" = stat_tab$elements) %>% distinct()
  write_csv(tukey_tab, paste0("S1_tukey.csv"))
...
eds_supp <- ggplot(
  ...
  facet_wrap(~elements, ncol = 1, scales = "free_y", strip.position = "left")

pdf("eds_plots_morpho.pdf", width = onecol * 0.5, height = onecol * 0.5)
eds_supp
dev.off()
...
rmn_bending <- rmn_sum %>% 
  ...
  mutate(
    genotype = recode(genotype,
                      ...
                      "Col-0" = "WT",
                      "fah1" = "<i>fah1</i>"),
    genotype = ordered(genotype, levels = c("<i>cad4</i>x<i>5</i>", "<i>fah1</i>", "WT"))) %>% 
  ...
  mutate(value_WT = mean(value[genotype == "WT"])) %>% 
...
pdf("IRX_figure5.pdf", width = twocol, height = onecol)
patch +
