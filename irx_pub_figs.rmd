...
rmn_data_corrected <- rmn_data_corrected %>%
  ...
  mutate(
    ...
    scaled_max = case_when(
      wavenumber == 1664 ~ corrected.intensity / corrected.intensity[wavenumber == 1603],
...
rmn_area_letters <- letter_groups(rmn_sum_long %>% filter(variable == "Area"),
                                 value,
                                 cell.type,
                                 "tukey",
                                 print_position = "below",
...
rmn_lig <- rmn_violin("total_lignin") +
  labs(
    x = "",
    y = expression(paste("Lignin band intensity [AU]"))
  )

rmn_cellu <- rmn_violin("cellulose") +
  labs(
    x = "",
    y = expression(paste("Cellulose band intensity [AU]"))
  )
