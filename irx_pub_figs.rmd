...
#### export spectra for excel users ####
...
rmn_lignin_dist <- model_data_pop %>%
  ...
  pivot_longer(-c(genotype, replicate, technical, Distance, TE), 
               names_to = "variable", 
               values_to = "value") %>%
  mutate(variable = ordered(variable, levels = c("Lignin/Cellulose", "S/G", "G-CHO/G-CHOH"))) %>%
...
# dist_lig <- ggplot(
#   rmn_lignin_dist_pred %>% filter(str_detect(cell_type, "SV")),
#   aes(
#     x = Distance,
#     y = value,
#     colour = genotype,
...
rmn_irx_long <- rmn_irx %>%
  select(genotype:cell.type, n_v, Area, Perim., Circ., convexity, total_lignin, G_rel, GOH.G, S_rel, cellulose, mean.OD1, GCHO_rel, S.G, P_rel, Benz_rel) %>%
...
rmn_irx_long <- model_data_pop %>%
  ungroup() %>%
  select(genotype, cell_type, n_v, Area, Perim., Circ., Solidity, total_lignin, G_rel, GOH.G, S_rel, cellulose, wiesner_avg, GCHO_rel, S.G, P_rel, Benz_rel) %>%
...
pdf("IRX_FigureS1_pop.pdf", width = onecol * 1.5, height = twocol)
pop_S1
dev.off()
```
