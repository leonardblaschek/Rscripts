...
read_plus <- function(flnm) {
  read_tsv(flnm,
    ...
    mutate(
      ...
      technical = str_extract_all(TE, "[:alpha:]+"),
      TE = str_extract_all(TE, "[:digit:]+")
    ) %>%
    pivot_wider(
      id_cols = c(genotype, replicate, technical, TE),
...
rmn_pop_test <- ggplot(rmn_pop_scaled,
  ...
  geom_vline(xintercept = c(1334, 1120, 1095, 381)) +
...
convexity_poplar <- ggplot(rmn_pop_data,
       aes(x = genotype,
           y = Solidity)) +
  geom_quasirandom(
      aes(fill = genotype),
