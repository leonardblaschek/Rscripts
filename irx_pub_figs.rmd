...
wiesner.data <-
  read.csv("/data/Phloroglucinol/measurements_revisited.csv",
    skip = 2
  )
...
wiesner.data.SMX <- read.csv("file:///data/Phloroglucinol/measurements_SMX.csv",
  skip = 2
)
...
raman.heights <- read.csv("file:///data/Master/Summer project 16/phenotyping/phenotyping.csv") %>%
...
lig_peak <- rmn_pop_corrected %>%
  ...
  filter(round(wavenumber, digits = 0) %in% c(1550:1650)) %>%
  mutate(
...
rmn_pop_scaled <- rmn_pop_corrected %>%
  left_join(lig_peak) %>%
  group_by(genotype, replicate, technical, TE, wavenumber) %>%
  mutate(wavenumber = wavenumber + (1600 - peak_pos)) %>%
  group_by(genotype, replicate, technical, TE) %>%
  mutate(
...
rmn_pop_avg <- rmn_pop_scaled %>%
  ...
  group_by(genotype, replicate, wavenumber) %>%
  summarise(mean = mean(scaled))

rmn_pop_sum <- rmn_pop_scaled %>%
  group_by(genotype, replicate, technical, TE) %>%
  summarise(
    "unscaled_lignin" = (corrected.intensity[abs(wavenumber - 1603) == min(abs(wavenumber - 1603))][1] / 2) +
    corrected.intensity[abs(wavenumber - 1334) == min(abs(wavenumber - 1334))][1],
    "total_lignin" = (scaled[abs(wavenumber - 1603) == min(abs(wavenumber - 1603))][1] / 2) +
      scaled[abs(wavenumber - 1334) == min(abs(wavenumber - 1334))][1],
    "P_rel" = scaled_lig[abs(wavenumber - 1000) == min(abs(wavenumber - 1000))][1],
    "G_rel" = scaled_lig[abs(wavenumber - 1603) == min(abs(wavenumber - 1603))][1],
    "G_tot" = scaled[abs(wavenumber - 1603) == min(abs(wavenumber - 1603))][1],
    "GOH_rel" = scaled_lig[abs(wavenumber - 1664) == min(abs(wavenumber - 1664))][1],
    ...
    "cellulose" = 
      ...
      scaled[abs(wavenumber - 381) == min(abs(wavenumber - 381))][1] +
    ...
    "GCHO.GOH" = scaled_lig[abs(wavenumber - 1625) == min(abs(wavenumber - 1625))][1] /
      scaled_lig[abs(wavenumber - 1664) == min(abs(wavenumber - 1664))][1],
    "AUC" = mean(AUC)
  ) 


rmn_pop_test <- ggplot(rmn_pop_scaled,
       aes(x = wavenumber,
           y = scaled)) +
  scale_x_continuous(trans = "reverse") +
  geom_vline(xintercept = c(1603,1334, 1120, 1095, 1000, 380)) +
  # geom_line(data = filter(rmn_pop_scaled, genotype == "C4H"),
  #           aes(group = interaction(genotype, replicate, technical, TE),
  #               colour = replicate),
  #           alpha = 0.25,
  #           size = 0.1) +
  geom_line(aes(group = interaction(genotype, replicate, technical, TE),
                colour = genotype),
            alpha = 0.25,
            size = 0.1) +
...
dist_lig <- ggplot(
  rmn_lignin_dist %>% filter(cell_type == "SV"),
  aes(
    x = Distance,
    y = value,
    colour = genotype,
...
n_TEs <- rmn_pop_data %>%
  drop_na(genotype, Solidity, cell_type) %>%
...
pdf("poplar_P.pdf", width = onecol * 0.5, height = onecol * 0.5)
convexity_poplar
dev.off()
```
