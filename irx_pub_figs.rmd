...
model_data_pop <- rmn_pop_data %>%
  ...
  mutate(technical = case_when(!is.na(repeat_technical) ~ repeat_technical,
         ...
         TE = case_when(!is.na(repeat_TE) ~ repeat_TE,
                               TRUE ~ TE)) %>%
  # group_by(genotype, replicate) %>% # separate models by realtive distance distance per section
  group_by(genotype) %>% # separate models by total distance
  mutate(median_dist = median(Distance, na.rm = TRUE),
         bin = case_when(Distance > median_dist ~ "early",
                         TRUE ~ "recent")) %>%
...
wt_data_pop <- model_data_pop %>%
  filter(genotype == "WT") %>%
  unite("cell_type", cell_type, bin) %>%
  mutate(cell_type = case_when(str_detect(cell_type, fixed("PV")) ~ "PV",
                               cell_type == "SV_early" ~ "Early SV",
                               cell_type == "SV_recent" ~ "Recent SV")) %>%
...
rmn_violin <- function(x) {
  letters <- letter_groups(wt_data_pop %>% filter(variable == x),
    ...
    mutate(cell_type = ordered(cell_type, levels = c("PV", "Early SV", "Recent SV")))
...
pdf("IRX_figure_pop1.pdf", width = onecol, height = onecol)
plot_grid(
  celltypes_WT,
  bottom,
  labels = c("A", ""),
...
```{r figure 3, fig.showtext = TRUE, out.width = '100%', message = FALSE, eval = TRUE, echo = FALSE}
irx_data <-
  read.csv(paste0(datapath,"PhD/IRX/Poplar/2020-08_soil_poplar/pop_rmn_coords.csv"), sep = "\t")
# irx_data <- subset(irx_data, replicate == 1 & technical == 3)
irx_ref <- subset(irx_data, object == "ref", select = c(1, 2, 3, 6, 7, 27))
irx_ref$ref.y1 <- irx_ref$Y
irx_ref$ref.y2 <- irx_ref$Y
...
heights <- read_csv(paste0(datapath, "Master/Summer project 16/phenotyping/phenotyping.csv")) %>%
