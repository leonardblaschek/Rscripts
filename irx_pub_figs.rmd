...
bending_data_cad <- lapply(bending_files, read_plus) %>%
  ...
  mutate(
    ...
    genotype = factor(genotype, levels = c("WT", "fah1", "cad4xcad5"))
  ) %>%
  mutate_if(is.character, ~ as.numeric(str_replace(., fixed(","), fixed("."))))
...
read_stiffness <- function(flnm) {
  read_delim(flnm,
    ...
    mutate(
      experiment = factor("cad", levels = c("fah1", "cad")),
      genotype = factor("cad4xcad5", levels = c("WT", "fah1", "cad4xcad5"))
    ) %>%
...
# WT_gg <- ggplotify::as.ggplot(WT)
# 
# patch <-
#   (WT_gg | medium_stiffness | medium_breakpoint) /
...
rmn_violin <- function(x) {
  letters <- letter_groups(rmn_sum_long %>% filter(variable == x),
    ...
    "tukey",
    print_position = "below",
    print_adjust = ifelse(x == "Area", 0.4,
                          ifelse(x == "G_rel", 2,
                                 ifelse(x == "S_rel", 2, 1)))
  ) %>%
    mutate(cell.type = ordered(cell.type, levels = c("PX", "PMX", "SMX")))
...
rmn_irx_letters <- letter_groups(
  filter(rmn_irx_long, variable %in%
           c("Adj. vessels",
             ...
             "Convexity",
             "Perimeter [µm]")) %>%
    group_by(genotype, variable, cell.type) %>%
    filter(n() > 2),
  value,
  genotype,
  "tukey",
...
pdf("IRX_FigureS1.pdf", width = onecol * 1.5, height = onecol * 1.5)
rmn_irx_morph
dev.off()
```
