...
eds_data <- read_csv("/home/leonard/Documents/Uni/PhD/IRX/EDS/eds_data.csv",
  col_types = "ccnnn"
)
...
em_collapse_plot <- function(cond) {
  em_collapse <- ggplot(em_data_avg %>% filter(treatment == cond), aes(
    ...
    # geom_point(
    #   data = em_data_points %>% filter(treatment == cond &
    #     state != "Fully"),
    #   aes(
    #     group = experiment,
    #     y = proportion - 0.01
...
afm_data <- read_csv("/home/leonard/Documents/Uni/PhD/IRX/AFM/afm_PX.csv") %>%
...
bending_files <-
  list.files(
    path = "/home/leonard/Documents/Uni/PhD/IRX/Bending/media_raw/",
    pattern = "*.txt",
    recursive = TRUE,
    full.names = TRUE
  )
...
medium_curves <- lapply(bending_files, read_plus) %>%
  bind_rows() %>%
  mutate(filename = str_remove(filename, fixed("Stem ")),
...
biochem_data <- read_csv("/home/leonard/Documents/Uni/PhD/IRX/Bending/bending_biochem.csv") %>%
  group_by(genotype) %>%
  summarise(
  ...
  ungroup() %>%
  mutate(scaled.lig.py = scale_range(mean.lig.py),
...
# Hmisc::rcorr(bending_avg$modulus ~ bending_avg$mean.lig.tga)
# summary(lm(bending_avg$modulus ~ bending_avg$mean.lig.tga))
# summary(lm(bending_avg$modulus ~ bending_avg$mean.sg))
# summary(lm(bending_avg$modulus ~ bending_avg$mean.cho.choh))
# 
# summary(lm(bending_avg$max.stress ~ bending_avg$mean.lig.tga))
# summary(lm(bending_avg$max.stress ~ bending_avg$mean.sg))
# summary(lm(bending_avg$max.stress ~ bending_avg$mean.cho.choh))
...
WT <-
  rasterGrob(
    readPNG(
      "/home/leonard/Documents/Uni/PhD/IRX/Bending/Images/bending_adjusted.png"
    )
  )
...
spectra_export.switch <- left_join(read_tsv("/home/leonard/Documents/Uni/PhD/IRX/PMX_which_SMX.tsv",
  col_types = "cccc"
), rmn_data_corrected) %>%
  mutate(
...
rmn_files <-
  list.files(
    path = "/home/leonard/Documents/Uni/PhD/IRX/Poplar/2020-08_soil_poplar/",
    pattern = "TE(.*).txt",
    recursive = TRUE,
    full.names = TRUE
  )
...
read_plus <- function(flnm) {
  read_tsv(flnm,
    ...
    mutate(
      filename = str_remove(
        flnm,
        fixed("/home/leonard/Documents/Uni/PhD/IRX/Poplar/2020-08_soil_poplar//")
      )
    ) %>%
    separate(filename, into = c("genotype", "replicate", "TE"), sep = "([/-])") %>%
    mutate(TE = str_remove(TE, fixed(".txt"))) %>%
    pivot_wider(
      id_cols = c(genotype, replicate, TE),
      names_from = wavenumber,
      values_from = intensity
    ) %>%
    group_by(genotype, replicate, TE) %>%
    nest()
}
...
rmn_pop_corrected <- rmn_pop %>%
  ...
  select(-data) %>%
  unnest() %>%
  pivot_longer(-c(genotype, replicate, TE),
    names_to = "wavenumber",
...
rmn_pop_sum <- rmn_pop_scaled %>%
  group_by(genotype, replicate, technical, TE) %>%
  summarise(
    "unscaled_lignin" = (corrected.intensity[abs(wavenumber - 1603) == min(abs(wavenumber - 1603))] / 2) +
      corrected.intensity[abs(wavenumber - 1334) == min(abs(wavenumber - 1334))],
    "total_lignin" = (scaled[abs(wavenumber - 1603) == min(abs(wavenumber - 1603))] / 2) +
      scaled[abs(wavenumber - 1334) == min(abs(wavenumber - 1334))],
    "G_rel" = scaled_lig[abs(wavenumber - 1603) == min(abs(wavenumber - 1603))],
    "G_tot" = scaled[abs(wavenumber - 1603) == min(abs(wavenumber - 1603))],
    "GOH_rel" = scaled_lig[abs(wavenumber - 1664) == min(abs(wavenumber - 1664))],
    "GCHO_rel" = scaled_lig[abs(wavenumber - 1625) == min(abs(wavenumber - 1625))],
    "GCHO_tot" = scaled[abs(wavenumber - 1625) == min(abs(wavenumber - 1625))],
    "S_rel" = scaled_lig[abs(wavenumber - 1334) == min(abs(wavenumber - 1334))],
    "S_tot" = scaled[abs(wavenumber - 1334) == min(abs(wavenumber - 1334))],
    "cellulose" = 
      scaled[abs(wavenumber - 1099) == min(abs(wavenumber - 1099))] +
      scaled[abs(wavenumber - 381) == min(abs(wavenumber - 381))],
    "lig.cellu" = total_lignin / cellulose,
    "G.cellu" = G_tot / cellulose,
    "S.G" = scaled_lig[abs(wavenumber - 1334) == min(abs(wavenumber - 1334))] /
      scaled_lig[abs(wavenumber - 1276) == min(abs(wavenumber - 1276))],
    # "GOH.G" = scaled_lig[wavenumber == 1664] / scaled_lig[wavenumber == 1603],
    "GCHO.GOH" = scaled_lig[abs(wavenumber - 1625) == min(abs(wavenumber - 1625))] /
      scaled_lig[abs(wavenumber - 1664) == min(abs(wavenumber - 1664))],
    "AUC" = mean(AUC)
  ) %>%
  ungroup() %>%
  mutate(
    technical = unlist(technical),
    TE = unlist(TE)
  )
...
#   #            linetype = 2) +
#   # geom_vline(xintercept = 381,
#   #            linetype = 2) +
#   # geom_vline(xintercept = 350,
#   #            linetype = 2) +
#   geom_line(aes(group = interaction(technical, replicate, TE)),
#             alpha = 0.25) +
...
pop_irx <- read_csv("/home/leonard/Documents/Uni/PhD/IRX/Poplar/2020-08_soil_poplar/pop_rmn_irx.csv")
...
rmn_pop_data <- left_join(rmn_pop_sum, pop_irx)

cor.test(
  rmn_pop_data$lig.cellu[rmn_pop_data$cell_type == "SV"],
  rmn_pop_data$Distance[rmn_pop_data$cell_type == "SV"]
)

rmn_lignin_dist <- rmn_pop_data %>%
  select(genotype, replicate, cell_type, Distance, 
  ...
  drop_na() %>%
  pivot_longer(-c(genotype:Distance), names_to = "variable", values_to = "value")

regressions <- rmn_lignin_dist %>%
  filter(cell_type == "SV") %>%
  select(-replicate) %>%
  nest(data = c(Distance, value)) %>% 
  mutate(
    fit = map(data, ~ lm(value ~ Distance, data = .x)),
    tidied = map(fit, glance),
    Distance = map_dbl(data, ~ max(.x$Distance)),
    value = map_dbl(data, ~ max(.x$value))
  ) %>% 
  unnest(tidied) %>% 
  select(-data, -fit) 
...
dist_lig <- ggplot(
  ...
  ggtext::geom_richtext(
    ...
    family = "Helvetica",
    fill = NA,
    aes(label = paste0("R<sup>2</sup> = ", round(adj.r.squared, 2), ", <i>P</i> = ", format(p.value, scientific = TRUE, digits = 2)))
  ) +
  theme_leo() +
  theme(axis.title.y = element_blank()) +
  labs(x = "Distance [px]") +
...
s_data <- read_csv("/home/leonard/Documents/Uni/PhD/IRX/Bending/stiffness_s.csv") %>%
  bind_rows(read_csv("/home/leonard/Documents/Uni/PhD/IRX/Bending/cad_diameters.csv")) %>%
  tidyr::fill(everything()) %>%
  mutate(
    genotype = ordered(genotype, levels = c("WT", "fah1", "cad4xcad5"))
  )
