...
pop_irx <- read_csv(
  "/data/PhD/IRX/Poplar/2020-08_soil_poplar/Wiesner/2020-10-22_poplar/registered/2020-11_pop_wiesner_irx.csv"
  ) %>%
  mutate(x = x / 5.84,
         y = y / 5.84)
...
wiesner_na <- read_csv("/data/PhD/IRX/Poplar/2020-08_soil_poplar/Wiesner/2020-10-22_poplar/registered/missing_wiesner.csv") %>%
  unite("ID", everything()) %>%
  as.list() %>%
  unlist()

wiesner_bg <- read_csv("/data/PhD/IRX/Poplar/2020-08_soil_poplar/Wiesner/2020-10-22_poplar/registered/wiesner_bg.csv") %>%
...
rmn_pop_n <- rmn_pop_data %>%
  # mutate(bin = case_when(Distance < 250 ~ 1,
  #                        TRUE ~2)) %>%
...
letters <- letter_groups(wiesner_pop_avg, 
                         wiesner_avg,
                         genotype,
                         "tukey",
                         cell_type,
...
ggplot(wiesner_pop_avg) +
  geom_violin(aes(x = genotype, y = wiesner_avg),
              draw_quantiles = 0.5) +
  geom_quasirandom(data = wiesner_pop_avg,
                   aes(x = genotype, y = wiesner_avg, colour = replicate),
                   alpha = 0.25) +
...
model_data_pop <- rmn_pop_data %>%
  ...
  mutate(max_dist = max(Distance, na.rm = TRUE),
         bin = case_when(Distance > (max_dist / 2) ~ "early",
                         TRUE ~ "recent")) %>%
  group_by(genotype, replicate, technical, cell_type, TE, bin) %>%
  summarise(across(where(is.numeric), ~ mean(.x, na.rm = TRUE)))
  
  
  
model_sv_pop <- model_data_pop %>%
  filter(cell_type == "SV") %>%
  ungroup() %>%
  select(
    Solidity,
...
# Fit the full model 
full_model <- lm(Solidity ~ ., data = model_sv_pop %>% filter(bin == "early") %>% select(-bin))
# Stepwise regression model
step_model <- MASS::stepAIC(full_model, direction = "both", 
                      trace = FALSE)
summary(step_model)

full_model <- lm(Solidity ~., data = model_sv_pop %>% filter(bin == "recent") %>% select(-bin))
# Stepwise regression model
step_model <- MASS::stepAIC(full_model, direction = "both", 
                      trace = FALSE)
summary(step_model)
...
psem_data_recent <- model_data_pop %>%
  ...
  group_by(genotype, replicate) %>%
  mutate(max_dist = max(Distance, na.rm = TRUE),
         bin = case_when(Distance > (max_dist / 2) ~ "recent",
                         TRUE ~ "recent")) %>%
  ungroup() %>%
  select(
    Solidity,
