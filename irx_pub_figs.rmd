...
rmn_files <-
  list.files(
    path = "/run/media/leonard/LEONARD 1TB/Grsync/office//PhD/IRX/Poplar/2020-08_soil_poplar",
...
read_plus <- function(flnm) {
  read_tsv(flnm,
    ...
    mutate(
      filename = str_remove(
        flnm,
        fixed("/run/media/leonard/LEONARD 1TB/Grsync/office//PhD/IRX/Poplar/2020-08_soil_poplar/")
      )
    ) %>%
...
rmn_pop <- lapply(rmn_files, read_plus) %>%
  ...
  ungroup() %>%
  select(-cell_type) %>%
  anti_join(read_csv("/run/media/leonard/LEONARD 1TB/Grsync/office//PhD/IRX/Poplar/2020-08_soil_poplar/pop_rmn_exclude.csv",
    col_types = "cccc"
  ))
...
pop_irx <- read_csv(
  "/run/media/leonard/LEONARD 1TB/Grsync/office/PhD/IRX/Poplar/2020-08_soil_poplar/pop_rmn_coords.csv"
  )
...
celltypes <- read_csv(
  "/run/media/leonard/LEONARD 1TB/Grsync/office/PhD/IRX/Poplar/2020-08_soil_poplar/poplar_cell_types.csv",
  col_types = "cccccc")
...
rmn_pop_n <- rmn_pop_data %>%
  mutate(bin = case_when(Distance < 250 ~ 1,
                         TRUE ~2)) %>%
  group_by(genotype, cell_type, bin) %>%
  summarise(n = n()) %>%
  ungroup() %>%
...
psem_data_pop <- rmn_pop_data %>%
  ungroup() %>%
  filter(cell_type == "SV" & Distance < 250) %>%
  select(
    Solidity,
    # n_v,
    # Perim.,
