...
pdf("afm_ratios.pdf", width = onecol * 0.715, height = onecol * 0.7)
...
flex_violin <- function(x) {
  ...
  geom_violin(
    ...
    scale_x_discrete(labels = rev(c("WT", 
...
biochem <- ggplot(biochem_data,
                  aes(x = genotype)) +
  geom_point(aes(size = lignin,
                 y = 1,
                 colour = genotype),
             alpha = 0.5,
             shape = 16) +
  geom_point(aes(size = s.g,
                 y = 2,
                 colour = genotype),
             alpha = 0.5,
             shape = 16) +
  geom_text(aes(label = round(lignin * 100, digits = 0),
                y = 1),
            family = "Helvetica",
            size = ggtext_size,
            hjust = 0.5) +
  geom_text(aes(label = round(s.g, digits = 2),
                y = 2),
            family = "Helvetica",
            size = ggtext_size,
            hjust = 0.5) +
  ...
  scale_y_continuous(limits = c(0.5, 2.5),
                     breaks = c(1, 2),
                     labels = c("Total\nlignin [%]","S/G\nratio")) +
  scale_colour_manual(values = rev(c(pal_ostwald_disc[1], 
                                 pal_ostwald_disc[3], 
                                 pal_ostwald_disc[3], 
                                 pal_ostwald_disc[3]))) +
...
pdf("IRX_figure5_recover.pdf", width = onecol, height = onecol * 0.85)
plot_grid(top,
  bottom,
  ncol = 1,
  labels = c("A", ""),
...
rmn_s <- rmn_violin("S_rel") +
  labs(
    x = "",
    y = expression(paste("Relative S-units"))
  )

rmn_g <- rmn_violin("G_rel") +
  labs(
    x = "",
    y = expression(paste("Relative G-units"))
  )
...
p_values <- pop_irx_overview %>% filter(variable == x) %>%
    summarise(p_value = broom::tidy(
  ...
  ggplot(
    ...
    scale_x_discrete(labels = c("PX", "SX")) +
    scale_fill_manual(values = pal_flame_disc[c(1, 3)]) +
    scale_y_continuous(expand = expand_scale(mult = c(0.15, 0.05))) +
    expand_limits(y = 0) +
    theme_leo() +
...
celltypes_WT <-
  rasterGrob(readPNG(
    "/home/leonard/Dropbox/IRX manuscript/Edits/poplarWT.png"
  ))
...
# ) +
  ...
  geom_smooth(data = filter(bending_data_complete, 
                            genotype == "WT" &
                              strain < 0.25),
              aes(x = strain,
                  y = stress),
              ...
              linetype = 2,
              size = 0.2) +
  geom_line(data = filter(bending_data_complete, genotype == "WT"),
            aes(x = strain, y = 13.25),
            linetype = 2,
            colour = "black",
            size = 0.2) +
  geom_text(data = filter(bending_data_complete, genotype == "WT")[1, ],
            aes(x = 3, y = 13.75, label = str),
            hjust = 1,
            vjust = 0,
            family = "Helvetica",
            size = ggtext_size) +
  geom_text(data = filter(bending_data_complete, genotype == "WT")[1, ],
            aes(x = 3, y = 82.5, label = stiff),
            hjust = 1,
            vjust = 0,
            angle = 44,
            family = "Helvetica",
            size = ggtext_size) +
  scale_colour_manual(values = c(pal_ostwald_disc[1], 
                                 pal_ostwald_disc[3], 
                                 pal_ostwald_disc[3], 
                                 pal_ostwald_disc[3])) +
  theme_leo() +
...
pdf("bending_curves.pdf", width = onecol, height = onecol)
curves
dev.off()
```
...
s_data <- read_csv("/home/leonard/Documents/Uni/PhD/IRX/Bending/stiffness_s.csv") %>%
  ...
  pivot_longer(cols = c(stiffness, strength), values_to = "value", names_to = "variable") %>%
  mutate(
    genotype = ordered(genotype, levels = c("WT", "fah1")),
    variable = recode(variable,
      "stiffness" = "Stiffness",
      "strength" = "Strength"
    )
