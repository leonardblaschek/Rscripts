...
pal_wes <- wes_palette("Zissou1", 100, type = "continuous")
pal_ostwald <- c("#155DA7", "#0A75B9", "#0C89C9", "#FED32F", "#F8A63A", "#EF663A", "#ED4137")
...
rmn_data_corrected <- rmn_data_corrected %>%
  ...
  mutate(
    ...
    scaled_max = case_when(
      wavenumber == 1625 ~ corrected.intensity / corrected.intensity[wavenumber == 1603],
...
rmn_sum <- rmn_data_corrected %>%
  group_by(genotype, cell.type, replicate, technical) %>%
  summarise(
    "total_lignin" = scaled[wavenumber == 1603] + 4.6 * scaled[wavenumber == 1334],
    "GOH_rel" = scaled_max[wavenumber == 1664],
    "GOH_tot" = scaled[wavenumber == 1664],
    "G_tot" = scaled[wavenumber == 1603],
    "G_rel" = scaled_max[wavenumber == 1603],
    "S.G" = scaled[wavenumber == 1334] / scaled[wavenumber == 1276],
    "GCHO.G" = scaled_max[wavenumber == 1625],
    "S_rel" = scaled_max[wavenumber == 1334],
...
rmn_sum_long <- rmn_sum %>%
  ...
  transmute(
    `Lignin band intensity` = total_lignin,
    `Cellulose band intensity` = cellulose,
    `Relative G-lignin` = G_rel,
    `Relative S-lignin` = S_rel,
    `Relative CHO-lignin` = GCHO.G
  ) %>%
  left_join(rmn_irx %>%
...
irx.summary <- ggplot(
  ...
  scale_y_continuous(
    ...
    # breaks = pretty_breaks(n = 4, min.n = 4)
    breaks = my_breaks
  ) +
  labs(
    x = "",
...
lig.summary <- ggplot(
  ...
  scale_y_continuous(
    limits = c(0, NA),
    # expand = expand_scale(mult = c(0.2, 0.05)),
    # breaks = function(value){seq(floor(min(value)), ceiling(max(value)), length = 4)}
...
ggplot() +
  ...
  scale_colour_gradientn(colours = pal_ostwald, trans = "reverse") +
...
# raman.irx.PMX <- filter(raman.irx, cell.type == "SMX") %>%
#   select(
#     convexity,
#     Perim.,
#     n_f,
...
# 
# 
# # chart.Correlation(raman.irx.PMX,
# #                   histogram=TRUE, pch=21)
# 
# model.null <- lm(convexity ~ 1,
#   data = raman.irx.PMX
# )
# model.full <- lm(convexity ~ .,
#   data = raman.irx.PMX
# )
# 
# step(model.null,
...
```{r, fig.showtext = TRUE, out.width = '100%', message = FALSE, eval = TRUE, echo = FALSE}
raman.irx.scaled <- raman.irx %>%
  select(-c(1:4)) %>%
  as.matrix() %>%
  scale() %>%
  as_tibble() %>%
  cbind(raman.irx[, c(1:4)])

model <- lme4::lmer(convexity ~
    n_v +
      mean.OD1 +
      Perim. +
      S_rel +
