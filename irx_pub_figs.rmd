...
eds_plot <- function(num, den, lbl) {
  ...
  p_values <- eds_data %>%
    ...
    mutate(p_value = case_when(p_value > 0.001 ~ round(p_value, digits = 3),
                               TRUE ~ round(p_value, digits = 4))
    )
...
pdf("eds_plots.pdf", width = onecol * 0.7, height = onecol / 2)
plot_grid(eds_plot(C, O) + theme(axis.text.x = element_blank(),
...
em_collapse <- ggplot(em_data_avg %>% filter(treatment == "AD"), aes(
  x = days,
  y = proportion,
  # group = experiment,
  fill = state
  ...
  geom_point(
    data = em_data_points %>% filter(treatment == "AD" &
      state != "Fully"),
    aes(
      group = experiment,
      y = proportion - 0.01
...
afm_data <- afm_data %>% bind_rows(afm_mx)
...
pdf("afm_ratios.pdf", width = onecol * 0.7, height = onecol * 0.75)
