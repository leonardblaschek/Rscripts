...
kruskal_tab <- stat_tab$kruskal %>% 
  add_column("variable" = "Adjacency to TEs [%]") %>% 
  distinct()
...
stat_tab <- rmn_irx_long %>% 
  ...
  anova_tab <- stat_tab$anova %>% 
    ...
    distinct()
  write_csv(anova_tab, paste0(stat_path, "S04A_anova", ".csv"))

  tukey_tab <- stat_tab$tukey %>%
    add_column("variable" = stat_tab$variable,
               "group" = stat_tab$cell.type) %>% 
    distinct()
  write_csv(tukey_tab, paste0(stat_path, "S04A_tukey", ".csv"))
...
stat_tab <- rmn_irx_long %>% 
  ...
  anova_tab <- stat_tab$kruskal %>% 
    add_column("variable" = stat_tab$variable) %>% 
    distinct()
  write_csv(anova_tab, paste0(stat_path, "S04B_kruskal", ".csv"))

  tukey_tab <- stat_tab$dunn %>%
    add_column("variable" = stat_tab$variable) %>% 
    distinct()
  write_csv(tukey_tab, paste0(stat_path, "S04B_dunn", ".csv"))
...
stat_tab <- biochem_data %>%
  ...
  anova_tab <- stat_tab$anova %>% 
    add_column("variable" = stat_tab$variable) %>% 
    distinct()
  write_csv(anova_tab, paste0(stat_path, "S07G_anova.csv"))

  tukey_tab <- stat_tab$tukey %>%
    add_column("variable" = stat_tab$variable) %>% 
    distinct()
  write_csv(tukey_tab, paste0(stat_path, "S07G_tukey.csv"))
