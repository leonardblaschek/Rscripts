...
doc_path <- "/run/media/leonard/data/grsync/Documents/"

stat_path <- "/home/leonard/Dropbox/2020_IRX manuscript/Submission/2022-06_PlantCell_revisions/supplementary/statistics/"
...
eds_plot <- function(num, den, lbl) {
  ...
  write_csv(anova_tab, paste0(stat_path, "01C_anova_", quo_name(num), "-", quo_name(den), ".csv"))
  
  tukey_tab <- stat_tab$tukey %>% distinct() %>% 
    mutate("variable" = paste0(quo_name(num), "/", quo_name(den)))
  write_csv(tukey_tab, paste0(stat_path, "01C_tukey_", quo_name(num), "-", quo_name(den), ".csv"))
...
rmn_violin <- function(x) {
  ...
  write_csv(p_values %>% rename("group" = cell.type), paste0(stat_path, "01E_t_", x, ".csv"))
...
em_data <- read_csv("/home/leonard/Dropbox/2020_IRX manuscript/Submission/2022-06_PlantCell_revisions/data/CPD_collapse_3.csv") %>%
  pivot_longer(cols = c(Fully, Partially, Intact), names_to = "state", values_to = "count") %>%
  mutate(
...
write_csv(em_data, "em_data_final.csv")
...
em_collapse_plot <- function(cond) {
  em_collapse <- ggplot(em_data_avg %>% filter(treatment == cond), aes(
    ...
    scale_y_continuous(expand = expand_scale(mult = c(0.01, 0.01))) +
    theme_leo() +
    labs(
      y = "Proportion",
      x = "Days after induction"
...
#### import and tidy Raman data (done once and written to file to save re-run time) ####
# rmn_files <-
#   list.files(
...
wiesner.data <-
  read.csv(paste0(doc_path, "Uni/Phloroglucinol/measurements_revisited.csv"),
    skip = 2
  )
...
wiesner.data.SMX <- read.csv(paste0(doc_path, "Uni/Phloroglucinol/measurements_SMX.csv"),
  skip = 2
)
...
raman.heights <- read.csv(paste0(doc_path, "Uni/Master/Summer project 16/phenotyping/phenotyping.csv")) %>%
...
kruskal_tab <- stat_tab$kruskal %>% 
  add_column("variable" = "Cell types adjacent to TEs [%]") %>% 
  distinct()
write_csv(kruskal_tab, paste0(stat_path, "03B_kruskal.csv"))

dunn_tab <- stat_tab$dunn %>% 
  add_column("variable" = "Cell types adjacent to TEs [%]") %>% 
  distinct()
write_csv(dunn_tab, paste0(stat_path, "03B_dunn.csv"))
...
rmn_adj_box <- ggplot(
  ...
  scale_x_discrete(labels = c("XP", "XF", "TEs")) +
  scale_fill_manual(values = pal_flame_disc) +
  labs(
...
rmn_violin <- function(x) {
  ...
  anova_tab <- stat_tab$anova %>% 
    ...
    distinct()
  write_csv(anova_tab, paste0(stat_path, "03", x_panel, "_anova.csv"))

  tukey_tab <- stat_tab$tukey %>%
    add_column("variable" = x) %>% 
    distinct()
  write_csv(tukey_tab, paste0(stat_path, "03", x_panel,"_tukey.csv"))
...
stat_tab <- irx_plot_data %>% 
  ...
  anova_tab <- stat_tab$anova %>% 
    ...
    distinct()
  write_csv(anova_tab, paste0(stat_path, "04C_anova", ".csv"))

  tukey_tab <- stat_tab$tukey %>%
    add_column("variable" = stat_tab$variable) %>% 
    distinct()
  write_csv(tukey_tab, paste0(stat_path, "04C_tukey", ".csv"))
...
psem_pmx <- psem(
  ...
  n_v +
    convexity,
  data = filter(drop_na(rmn_irx), cell.type == "MX")
  ),
  lm(convexity ~
  n_v +
    # n_f +
...
psem_smx <- psem(
  ...
  # n_v +
  convexity,
  data = filter(drop_na(rmn_irx), cell.type == "SX")
  ),
  lm(convexity ~
  # n_v +
  # n_f +
...
rmn_violin <- function(x) {
  ...
  anova_tab <- stat_tab$anova %>% 
    add_column("variable" = x) %>% 
    distinct()
  write_csv(anova_tab, paste0(stat_path, "05", x_panel, "_anova.csv"))

  tukey_tab <- stat_tab$tukey %>%
    add_column("variable" = x) %>% 
    distinct()
  write_csv(tukey_tab, paste0(stat_path, "05", x_panel,"_tukey.csv"))
...
#### calculate difference of the centre of each vessel to the cambium ####
# implemented according to https://stackoverflow.com/questions/35194048/using-r-how-to-calculate-the-distance-from-one-point-to-a-line
pop_irx$Distance <-
  apply(
    pop_irx[, c("X", "Y", "ref.x1", "ref.x2", "ref.y1", "ref.y2")],
    1,
...
flex_violin <- function(x, dat = medium_data) {
  ...
  anova_tab <- stat_tab$anova %>% 
    add_column("variable" = x) %>% 
    distinct()
  write_csv(anova_tab, paste0(stat_path, x_figure, x_panel, "_anova.csv"))

  tukey_tab <- stat_tab$tukey %>%
    add_column("variable" = x) %>% 
    distinct()
  write_csv(tukey_tab, paste0(stat_path, x_figure, x_panel,"_tukey.csv"))
...
stat_tab <- rmn_bending %>% 
  ...
  anova_tab <- stat_tab$anova %>% 
    add_column("variable" = stat_tab$variable) %>% 
    distinct()
  write_csv(anova_tab, paste0(stat_path, "06E_anova", ".csv"))

  tukey_tab <- stat_tab$tukey %>%
    add_column("variable" = stat_tab$variable) %>% 
    distinct()
  write_csv(tukey_tab, paste0(stat_path, "06E_tukey", ".csv"))
...
```{r, fig.showtext = TRUE, out.width = '100%', message = FALSE, eval = TRUE, echo = FALSE}
rosettes <- read_csv("/home/leonard/Dropbox/2020_IRX manuscript/Submission/2022-06_PlantCell_revisions/data/drought/drought_leaf_area.csv") %>%
  mutate(
    medium = ordered(medium, levels = c("Water", "10% PEG", "20% PEG")),
    genotype = ordered(genotype,
                       levels = c("WT", "<i>fah1</i>", "<i>cad4</i>x<i>5</i>", "<i>cad4</i>x<i>5</i>x<br><i>fah1</i>")),
    during_rel = (during / before * 100) - 100,
    after_rel = (after / before * 100) - 100
  )
  # filter(medium != "Water")
  # select(genotype, replicate, medium, during_rel, after_rel) %>% 
  # pivot_longer(c(during_rel, after_rel), names_to = "timepoint", values_to = "rel_area")
...
stat_tab <- rosettes %>%
  ...
  anova_tab <- stat_tab$anova %>%
    add_column(
      "variable" = "relative change in rosette area",
      "group" = stat_tab$medium) %>%
    distinct()
...
area_medium <- ggplot(
  ...
  scale_fill_manual(values = pal_ostwald_disc[c(1, 3, 3, 3)]) +
  scale_colour_manual(values = pal_ostwald_disc[c(1, 3, 3 ,3)]) +
  # scale_x_discrete(labels = function(x) str_wrap(x, width = 6)) +
  # expand_limits(y = 0) +
  labs(y = "Relative change in rosette area [%]") +
  theme_leo() +
  theme(
    axis.title.x = element_blank(),
...
transpiration <- read_csv(
  ...
  mutate(
    ...
    genotype = recode(genotype, "CAD" = "<i>cad4</i>x<i>5</i>"),
  ...
  select(-plant) %>%
  bind_rows(read_csv(
    "/home/leonard/Dropbox/2020_IRX manuscript/Submission/2022-06_PlantCell_revisions/data/drought/drought_evapotrans.csv",
    col_types = "ccccnn")) %>%
  filter(time < 10) %>%
  unite("replicate", instance, replicate) %>%
  mutate(
  ...
  left_join(rosettes %>% mutate(replicate = paste0("2_", replicate))) %>% 
  mutate(rel_transp = transp_rate / before)

transpiration_avg <- transpiration %>%
  group_by(genotype, replicate, medium) %>%
  summarise(mean_transp = mean(transp_rate, na.rm = TRUE),
            mean_rel_transp = mean(rel_transp, na.rm = TRUE)) %>% 
  drop_na(mean_rel_transp)
...
stat_tab <- transpiration_avg %>%
  ...
  anova_tab <- stat_tab$anova %>%
    add_column(
      "variable" = "evapotranspiration rate per cm² projected leaf area",
      "group" = stat_tab$medium) %>%
    distinct()
  write_csv(anova_tab, paste0(stat_path, "07C_anova", ".csv"))
  ...
  tukey_tab <- stat_tab$tukey %>%
    add_column(
      ...
      "group" = stat_tab$medium) %>%
    distinct()
  write_csv(tukey_tab, paste0(stat_path, "07C_tukey", ".csv"))
...
transpiration_medium <- ggplot(
  ...
  # scale_x_discrete(labels = function(x) str_wrap(x, width = 6)) +
  expand_limits(y = 0) +
  labs(y = "Evapotranspiration [mg / min cm<sup>2</sup>]") +
  theme_leo() +
  theme(
    axis.title.x = element_blank(),
    axis.title.y = element_textbox_simple(
...
pdf("transpiration_facet.pdf", width = onecol, height = onecol * 0.5)
...
```{r, fig.showtext = TRUE, out.width = '100%', message = FALSE, eval = TRUE, echo = FALSE}
recovery_data <- read_csv("/home/leonard/Dropbox/2020_IRX manuscript/Submission/2022-06_PlantCell_revisions/data/drought/drought_recovery.csv") %>%
  mutate(
    state = ordered(state, levels = rev(c("Recovered", "Partly recovered", "Dead"))),
    medium = ordered(medium, levels = c("Water", "10% PEG", "20% PEG")),
    genotype = ordered(recode(genotype,
      "cad4x5" = "<i>cad4</i>x<i>5</i>",
      "fah1" = "<i>fah1</i>",
      "cad4x5xfah1" = "<i>cad4</i>x<i>5</i>x<br><i>fah1</i>"
    ),
    levels = c("WT", "<i>fah1</i>", "<i>cad4</i>x<i>5</i>", "<i>cad4</i>x<i>5</i>x<br><i>fah1</i>")
    )
  ) %>%
  filter(medium != "Water") %>%
  group_by(genotype, medium, replicate) %>%
  mutate(sum = sum(count)) %>%
  group_by(genotype, medium, replicate, state) %>%
  mutate(proportion = (count / sum) * 100)

recovery_n <- recovery_data %>% 
  group_by(genotype, medium) %>% 
  summarise(n = sum(count))

recovery_avg <- recovery_data %>% 
  ...
  mutate(
    ymin = case_when(
      state == "Partly recovered" ~ proportion[state == "Recovered"] +
        proportion[state == "Partly recovered"] - sd,
      state == "Dead" ~ NaN,
      state == "Recovered" ~ proportion - sd
    ),
    ymax = case_when(
      state == "Partly recovered" ~ proportion[state == "Recovered"] +
        proportion[state == "Partly recovered"] + sd,
...
letters <- recovery_data %>%
  select(-c(count, sum)) %>%
  pivot_wider(names_from = state, values_from = proportion) %>%
  summarise(proportion = Recovered + `Partly recovered`,
            state = "Recovered") %>%
  letter_groups(.,
                proportion,
                genotype,
...
stat_tab <- recovery_data %>%
  ...
  anova_tab <- stat_tab$anova %>%
    add_column(
      "variable" = "proportion of plants that recovered at least partly",
      "group" = stat_tab$medium
      ) %>%
    distinct()
  write_csv(anova_tab, paste0(stat_path, "07D_anova", ".csv"))
  ...
  tukey_tab <- stat_tab$tukey %>%
    ...
    distinct()
  write_csv(tukey_tab, paste0(stat_path, "07D_tukey", ".csv"))


recovery_collapse <- ggplot(recovery_avg, aes(
  x = genotype,
  y = proportion,
  fill = state
)) +
...
pdf("recovery.pdf", width = onecol * 0.5, height = onecol * 0.5)
recovery_collapse
dev.off()
```
...
hyp_data <- read_csv("/home/leonard/Dropbox/2020_IRX manuscript/hypocotyl_collapse.csv") %>% 
  ...
  mutate(genotype = recode(genotype,
         ...
         condition = ordered(recode(condition, "0%" = "Water", "10%" = "10% PEG", "20%" = "20% PEG"),
         levels = rev(c("Water", "10% PEG", "20% PEG")))
  )
...
stat_tab <- hyp_avg %>% 
  ...
  anova_tab <- stat_tab$anova %>% 
    mutate(variable = "Convexity") %>% 
    distinct()
  write_csv(anova_tab, paste0(stat_path, "07E_anova.csv"))

  tukey_tab <- stat_tab$tukey %>%
    mutate(variable = "Convexity") %>% 
    distinct()
  write_csv(tukey_tab, paste0(stat_path, "07E_tukey.csv"))
...
### Assemble figure
```{r, fig.showtext = TRUE, out.width = '100%', message = FALSE, eval = TRUE, echo = FALSE}
patch <- area_medium + transpiration_medium + recovery_collapse + hyp_dist
...
pdf("drought_panel.pdf", width = onecol * 1.1, height = onecol * 1.1)
patch + 
  plot_layout(design = layout) &
  plot_annotation(
    tag_levels = list(c("B", "C", "D", "E")),
    theme = theme(plot.margin = margin(-3.5, -2, -2, -6.5))
  ) &
  theme(plot.tag = element_text(size = 10, face = "bold", family = "Helvetica"))
dev.off()
```
...
wt_V_spec <- ggplot(rmn_WT_V,
  ...
  geom_line(size = 0.2) +
  scale_x_reverse(expand = expansion(mult = 0)) +
  scale_colour_manual(values = pal_ostwald_disc[c(3, 1)]) +
...
pdf("V_spec.pdf", width = onecol * 1.3, height = onecol * 0.7) 
wt_V_spec
dev.off()
```
...
best_model <- find_interactions("MX")
...
best_model <- find_interactions("SX")
...
stat_tab <- rmn_irx_long %>% 
  ...
  anova_tab <- stat_tab$anova %>% 
    ...
    distinct()
  write_csv(anova_tab, paste0(stat_path, "S04B_anova", ".csv"))

  tukey_tab <- stat_tab$tukey %>%
    add_column("variable" = stat_tab$variable,
               "group" = stat_tab$cell.type) %>% 
    distinct()
  write_csv(tukey_tab, paste0(stat_path, "S04B_tukey", ".csv"))
...
pdf("IRX_adj_TE.pdf", width = twocol, height = onecol * 0.4)
adj_TE
dev.off()
```
...
stat_tab <- rmn_irx_long %>% 
  ...
  anova_tab <- stat_tab$kruskal %>% 
    add_column("variable" = stat_tab$variable) %>% 
    distinct()
  write_csv(anova_tab, paste0(stat_path, "S04C_kruskal", ".csv"))

  tukey_tab <- stat_tab$dunn %>%
    add_column("variable" = stat_tab$variable) %>% 
    distinct()
  write_csv(tukey_tab, paste0(stat_path, "S04C_dunn", ".csv"))
...
rmn_irx_lig <- ggplot(
  ...
  geom_density_ridges(colour = "black",
                      ...
                      quantile_lines = TRUE,
                      quantiles = 2) +
  geom_density_ridges(data = rmn_irx_long %>% filter(genotype == "Col-0"),
                      colour = "red",
                      fill = "grey95",
                      scale = 0.3,
                      size = 0.2,
...
pdf("IRX_FigureS1b.pdf", width = twocol, height = twocol)
adj_TE + rmn_irx_lig +
...
##### Neighbouring TEs and perimeters

```{r, fig.showtext = TRUE, out.width = '100%', message = FALSE, eval = TRUE, echo = FALSE}
rmn_irx_long <- model_data_pop %>%
  mutate(n_v = n_v * 100) %>%
  select(genotype, replicate, technical, cell_type, n_v, Perim.) %>%
...
pdf("IRX_adj_TE.pdf", width = twocol, height = onecol * 0.6)
adj_TE_pop
dev.off()
```
...
stat_tab <- rmn_irx_long %>% 
  ...
  anova_tab <- stat_tab$kruskal %>% 
    add_column("variable" = stat_tab$variable,
               "group" = stat_tab$cell_type) %>% 
    distinct()
  write_csv(anova_tab, paste0(stat_path, "S06C_kruskal", ".csv"))

  tukey_tab <- stat_tab$dunn %>%
    add_column("variable" = stat_tab$variable,
               "group" = stat_tab$cell_type) %>% 
    distinct()
  write_csv(tukey_tab, paste0(stat_path, "S06C_dunn", ".csv"))
...
poplar_variation <- ggplot(
  ...
  geom_density_ridges(colour = "black",
                      ...
                      quantile_lines = TRUE,
                      quantiles = 2) +
  geom_density_ridges(data = rmn_irx_long %>% filter(genotype == "WT"),
                      colour = "red",
                      fill = "grey95",
                      scale = 0.3,
                      size = 0.2,
...
rmn_lignin_dist <- model_data_pop %>%
  ...
  select(genotype, replicate, technical, Distance, TE,
         ...
         "bold(S)~'/'~bold(G)" = S.G,
         "bold(G)[CHO]~'/'~bold(G)[CHOH]" = GCHO.GOH,
         # "Convexity" = Solidity
         ) %>%
  drop_na() %>%
...
pdf("poplar_supp.pdf", width = twocol, height = twocol)
dist_lig1 + adj_TE_pop + poplar_variation +
  plot_layout(nrow = 3, heights = c(0.3, 0.3, 0.7)) &
  plot_annotation(tag_levels = 'A') & 
  theme(plot.tag = element_text(size = 10, face = "bold", family = "Helvetica"))
dev.off()
...
stat_tab <- medium_data %>% 
  ...
  anova_tab <- stat_tab$anova %>% 
    distinct()
  write_csv(anova_tab, paste0(stat_path, "S08C_anova.csv"))

  tukey_tab <- stat_tab$tukey %>%
    distinct()
  write_csv(tukey_tab, paste0(stat_path, "S08C_tukey.csv"))
...
stat_tab <- s_data %>% 
  ...
  anova_tab <- stat_tab$anova %>% 
    distinct()
  write_csv(anova_tab, paste0(stat_path, "S08F_anova.csv"))

  tukey_tab <- stat_tab$tukey %>%
    distinct()
  write_csv(tukey_tab, paste0(stat_path, "S08F_tukey.csv"))
...
biochem_data <- read_csv(paste0(datapath, "PhD/IRX/Bending/bending_biochem.csv")) %>% 
  ...
  full_join(read_csv("/run/media/leonard/data/grsync/Documents/Uni/Master/Summer project 16/phenotyping/phenotyping.csv") %>% mutate(genotype = recode(genotype, "col-0" = "WT"))) %>% 
...
heights <- read_csv("/run/media/leonard/data/grsync/Documents/Uni/Master/Summer project 16/phenotyping/phenotyping.csv") %>%
...
stat_tab <- biochem_data %>%
  ...
  anova_tab <- stat_tab$anova %>% 
    add_column("variable" = stat_tab$variable) %>% 
    distinct()
  write_csv(anova_tab, paste0(stat_path, "S08G_anova.csv"))

  tukey_tab <- stat_tab$tukey %>%
    add_column("variable" = stat_tab$variable) %>% 
    distinct()
  write_csv(tukey_tab, paste0(stat_path, "S08G_tukey.csv"))
...
pdf("transpiration_supplement.pdf", width = twocol, height = onecol * 0.8)
hum / ill / temp + plot_layout(heights = c(0.5, 0.5, 0.5 , 1))
dev.off()
...
```{r}
casp <- read_csv(paste0(datapath, "/PhD/IRX/Casparian_strip/distance_from_elongation.csv")) %>%
  drop_na(distance) %>% 
...
```{r}
casp_cells <- read_csv(paste0(datapath, "PhD/IRX/Casparian_strip/cells_from_elongation.csv")) %>%
  drop_na(cell) %>% 
...
casp_cell_size <- read_csv(paste0(datapath, "PhD/IRX/Casparian_strip/cells_from_elongation.csv")) %>%
  drop_na(cell) %>% 
  mutate(size = size / 2.4089) %>% # pixel dimension
  separate(file, into = c("genotype", "replicate")) %>% 
  mutate(genotype = ordered(genotype, levels = c("WT", "fah1", "cad4x5"))) %>% 
...
stat_tab <- casp_cells %>% 
  ...
  anova_tab <- stat_tab$anova %>% 
    mutate(variable = "cells to intact endodermis") %>% 
    distinct()
  write_csv(anova_tab, paste0(stat_path, "S09B_anova.csv"))

  tukey_tab <- stat_tab$tukey %>%
    mutate(variable = "cells to intact endodermis") %>% 
    distinct()
  write_csv(tukey_tab, paste0(stat_path, "S09B_tukey.csv"))
...
```{r}
gcho <- read_table("/home/leonard/Dropbox/2020_IRX manuscript/Submission/2022-06_PlantCell_revisions/data/MD/7mer-4O5-GCHO.dat",
                   comment = "#",
                   col_names = c("angle", "a-b", "b-O", "a-6")) %>% 
  mutate(oligomer = "<b>G</b><sub>CHO</sub>")

gchoh <- read_table("/home/leonard/Dropbox/2020_IRX manuscript/Submission/2022-06_PlantCell_revisions/data/MD/7mer-4O5-GCHOH.dat",
                   comment = "#",
                   col_names = c("angle", "a-b", "b-O", "a-6")) %>% 
  mutate(oligomer = "<b>G</b><sub>CHOH</sub>")

gchoha <- read_table("/home/leonard/Dropbox/2020_IRX manuscript/Submission/2022-06_PlantCell_revisions/data/MD/7mer-4O5-GCHOH-aOH.dat",
                   comment = "#",
                   col_names = c("angle", "a-b", "b-O", "a-6")) %>% 
  mutate(oligomer = "<b>G</b><sub>CHOH</sub> with <sup>&alpha;</sup>C-OH")

schoha <- read_table("/home/leonard/Dropbox/2020_IRX manuscript/Submission/2022-06_PlantCell_revisions/data/MD/7mer-4O5-SCHOH-aOH.dat",
                   comment = "#",
                   col_names = c("angle", "a-b", "b-O", "a-6")) %>% 
  mutate(oligomer = "<b>S</b><sub>CHOH</sub> with <sup>&alpha;</sup>C-OH")

torsions <- bind_rows(gcho, gchoh, gchoha, schoha) %>% 
...
torsion_plot <- ggplot(
  ...
  scale_colour_manual(values = pal_ostwald_disc[c(3, 1, 1, 2)]) +
...
```{r}
mut_csv <- read_csv("/home/leonard/Dropbox/2020_IRX manuscript/Submission/Plant Cell revised - revise/Supplemental/mut_table.csv")
...
stat_tables <- list.files(
  path = "/home/leonard/Dropbox/2020_IRX manuscript/Submission/2022-06_PlantCell_revisions/supplementary/statistics/",
  full.names = TRUE
)
...
write_csv(stat_table,
  "Menard2022_supplemental_stats.csv",
  na = ""
)
...
list_of_tables %>%
  writexl::write_xlsx(path = "Menard2022_supplemental_stats.xlsx")
...
bending_turgor <- medium_data %>% 
  ...
  pivot_wider(names_from = variable, values_from = value) %>% 
  select(-who_knows) %>% 
  rename("flexibility" = "break_point")
...
# Mutant biochemistry ####
biochem_data_clean <- biochem_data %>% 
  rowwise() %>% 
  mutate(variable = str_remove_all(strip_html(variable), fixed("*")))

# Cells to intact casparian strip
casp_cell_size <- read_csv(paste0(datapath, "PhD/IRX/Casparian_strip/cells_from_elongation.csv")) %>%
  drop_na(cell) %>% 
  mutate(size = size / 2.4089) %>% # pixel dimension
  separate(file, into = c("genotype", "replicate")) %>% 
...
rosettes_export <- read_csv("/home/leonard/Dropbox/2020_IRX manuscript/Submission/2022-06_PlantCell_revisions/data/drought/drought_leaf_area.csv") %>%
  mutate(
    medium = ordered(medium, levels = c("Water", "10% PEG", "20% PEG")),
    genotype = ordered(genotype,
                       levels = c("WT", "<i>fah1</i>", "<i>cad4</i>x<i>5</i>", "<i>cad4</i>x<i>5</i>x<br><i>fah1</i>")),
  ...
  select(genotype, replicate, medium, "Relative rosette area after recovery [%]" = after_rel) %>% 
  rowwise() %>%
  mutate(genotype = strip_html(genotype))
...
transpiration <- read_csv(
  ...
  left_join(rosettes %>% mutate(replicate = paste0("2_", replicate))) %>% 
  mutate("transpiration per cm² projected leaf area" = transp_rate / before) %>% 
  select(medium, time, genotype, `transpiration per cm² projected leaf area`) %>% 
  drop_na() %>% 
  rowwise() %>%
  mutate(genotype = strip_html(genotype))
...
writexl::write_xlsx(list(
  "EDS data" = eds_data,
  "EDS by morphotype" = eds_data_supp,
  "EM data" = em_data,
  "AFM data" = afm_data,
  ...
  "Mutant flexibility" = breaking_mutants,
  "Mutant biochemistry" = biochem_data_clean,
  "Casparian strip" = casp_cell_size,
  "Drought rosette area" = rosettes_export,
  "Drought evapotranspiration" = transpiration,
  "Drought recovery" = recovery_data,
  "Hypocotyl collapse" = hyp_data
),
path = "Menard2022_data.xlsx",
col_names = TRUE,
format_headers = TRUE,
use_zip64 = FALSE
)
