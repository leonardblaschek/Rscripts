...
read_plus <- function(flnm) {
  read_tsv(flnm,
    ...
    mutate(
      ...
      technical = str_extract_all(cell, "[:alpha:]+"),
      cell = str_extract_all(cell, "[:digit:]+"),
...
rmn_c4h_raw <- rmn_pop_scaled %>%
  mutate(wavenumber = round(wavenumber, digits = 0)) %>%
  pivot_wider(id_cols = c(genotype, replicate, technical, cell_type, cell),
              names_from = wavenumber,
              values_from = corrected.intensity)

rmn_c4h_raw <- rmn_c4h_raw %>%
  select(genotype, replicate, technical, cell_type, cell, order(as.numeric(colnames(.))))
