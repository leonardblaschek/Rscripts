...
ggplot(
  ...
  geom_label_repel(
    ...
    aes(
      label = c("S-lignin", "Aldehydes", "G-lignin"),
      y = mean.scaled
    ),
...
```{r model compounds, fig.showtext = TRUE, out.width = '100%', warning = FALSE, message = FALSE, echo = FALSE, fig.cap = "**Figure 1**: "}
...
geom_text_repel(data = tibble(wavenumber = rep(c(1656, 1621, 1600, 1335, 1140), 2),
          ...
          size = 6 / (14/5)) +
  annotate("segment",
           x = c(1656, 1621, 1600, 1335, 1140),
           xend = c(1656, 1621, 1600, 1335, 1140),
           y = 1,
           yend = Inf,
           colour = "grey85") +
  geom_ridgeline(aes(
...
pdf("peak_pres_grid.pdf", width = 6.7, height = 3)
spectra_plot
dev.off()
...
peak_pres <- read_csv("/home/leonard/Documents/Uni/PhD/Raman/synth_peak_presence.csv") %>%
  ...
  mutate(compound = ordered(compound,
    levels =
      rev(c(
        "poly-g-oh",
        "mono-g-oh",
...
peak_upset <- ggplot(peak_pres) +
  ...
  scale_colour_manual(values = c("yes" = "black", "no" = "grey85")) +
