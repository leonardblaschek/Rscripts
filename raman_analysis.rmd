...
rmn_pres <- rmn_data_avg_WT %>%
  filter(cell.type == "PMX") %>%
  mutate(fill_col = case_when(wavenumber %in% c(1550:1640) ~ "lignin",
                               wavenumber %in% c(1080:1115) ~ "cellulose"))

pdf("pres_spectra.pdf", height = 4, width = 10)
ggplot(
  data = rmn_pres,
  aes(
    x = wavenumber,
  ...
  geom_ridgeline_gradient(aes(fill = fill_col),
    min_height = -0.5
  ) +
  scale_fill_few(na.value = rgb(0, 0, 0, 0)) +
  geom_label_repel(
    data = filter(rmn_pres, wavenumber %in% c(1664, 1625, 1340)),
    aes(label = c("S-lignin", "Aldehydes", "G-lignin"),
        y = mean.scaled),
  ...
  annotate(geom = "text", x = 1099, y = 0.2, hjust = 1, label = "Cellulose", family = "Helvetica", size = 5) +
  annotate(geom = "text", x = 1593, y = 0.95, hjust = 1, label = "Lignin", family = "Helvetica", size = 5) +
  labs(x = "Wavenumber") +
  theme_leo() +
...
Here are the spectra from Zoltan. My first take-aways are the following:

* **1625 cm^-1^** comes from S and G aldehydes and ferulates
* **1664 cm^-1^** comes from G-OH > G-CHO = S-OH = S-CHO
* **1600 cm^-1^** shifts toward lower wavenumbers the more carbons are directly linked to the aromatic ring, *i.e* monomers > polymers and C > G > S
* **1340 cm^-1^** comes from all S-units
* **1130 cm^-1^** comes from aldehydes

```{r model compounds, fig.showtext = TRUE, out.width = '100%', warning = FALSE, message = FALSE, echo = FALSE, fig.cap = "**Figure 6**: Spectra of monomeric and polymeric lignin model compounds. The intensities are scaled relative to the total area under the curve of the respective spectrum. For assignments of the annotated wavenumbers see table 2."}
...
rmn_models <- rmn_models %>%
  ...
  mutate(AUC = MESS::auc(wavenumber, intensity),
         scaled = intensity / AUC)

# pdf("monomer_spectra.pdf", height = 5, width = 9)
ggplot(rmn_models) +
  geom_vline(xintercept = 1625, linetype = 2) +
  geom_vline(xintercept = 1664, linetype = 2) +
  geom_vline(xintercept = 1340, linetype = 2) +
  geom_vline(xintercept = 1383, linetype = 2) +
  geom_vline(xintercept = 1150, linetype = 2) +
  geom_vline(xintercept = 1130, linetype = 2) +
  annotate("text", label = "1625", x = 1620, y = 0.75, hjust = 0, family = "Helvetica") +
  annotate("text", label = "1664", x = 1670, y = 0.75, hjust = 1, family = "Helvetica") +
  annotate("text", label = "1340", x = 1335, y = 0.75, hjust = 0, family = "Helvetica") +
  annotate("text", label = "1383", x = 1387, y = 0.75, hjust = 1, family = "Helvetica") +
  annotate("text", label = "1150", x = 1155, y = 0.75, hjust = 1, family = "Helvetica") +
  annotate("text", label = "1130", x = 1125, y = 0.75, hjust = 0, family = "Helvetica") +
  geom_ridgeline(aes(
    x = wavenumber,
    y = filename,
    height = scaled,
  ...
  scale_fill_viridis_d() +
  scale_x_reverse() +
  scale_y_discrete(labels = c("C-COOH",
                              "G-CHO",
                              "G-COOH",
                              "G-OH",
                              "S-CHO",
