...
exp_data <- model_data_modeller %>% 
  ...
  mutate(kingdom = case_when(is.na(kingdom) ~ "plantae",
                             TRUE ~ kingdom),
         model = case_when(paralog == "1KYA (X-ray)" ~ "crystallography",
                           model == "relaxed" ~ "AlphaFold 2",
                           model == "6klg" ~ "crystallography",
                           TRUE ~ model),
         DOPE = case_when(paralog == "ZmLAC3" ~ -1.740,
                          TRUE ~ DOPE)) %>% 
  pivot_wider(names_from = parameter, values_from = value)
