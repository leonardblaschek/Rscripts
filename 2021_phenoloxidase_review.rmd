...
ex_plot <- ggplot(expression_count, aes(x = compartment, y = number, fill = PO)) +
  ...
  annotate("rect",
           ...
           fill = "grey95",
           colour = NA) +
  geom_jitter(data = filter(expression_count, method == "pred"), aes(x = compartment, y = number, fill = PO),
              width = 0.2,
              shape = 21,
              alpha = 0.5,
              size = 0.5,
              stroke = 0.1) +
  geom_jitter(data = filter(expression_count, method == "exp"), aes(x = compartment, y = number, fill = PO),
              width = 0.2,
              shape = 21,
              alpha = 0.75,
              size = 1,
...
model_data <- his_depth %>% 
  ...
  left_join(pocket_data %>%  select(paralog, model, Vol_sa, mouth_area_sa)) %>%
  pivot_longer(c(HIS_451, HIS_519, Vol_sa, mouth_area_sa), names_to = "parameter", values_to = "value")

ref_data <- zmlac3_depth %>%
  select(paralog, model, `6klg`, `mean(depth)`) %>%
  pivot_wider(names_from = `6klg`, values_from = `mean(depth)`) %>% 
  left_join(zmlac3_pocInfo %>%  select(paralog, model, Vol_sa)) %>%
  left_join(zmlac3_mouthInfo %>%  select(paralog, model, mouth_area_sa)) %>%
  pivot_longer(c(HIS_451, HIS_519, Vol_sa, mouth_area_sa), names_to = "parameter", values_to = "value")

axis_titles <- c(HIS_451 = "Depth of His 451 [Å]",
                 HIS_519 = "Depth of His 519 [Å]",
                 Vol_sa = "Binding pocket volume [Å<sup>3</sup>]",
                 mouth_area_sa = "Binding pocket mouth [Å<sup>2</sup>]")
...
clust <- reorder(hclust(dist(clust_data), method = "average"), -clust_data$HIS_451, agglo.FUN = "mean")
...
violins <- ggplot(model_data,
       aes(x = paralog,
           y = value)) +
  geom_hline(data = ref_data,
  ...
  geom_violin(draw_quantiles = 0.5,
              ...
              colour = "black",
              size = 0.2) +
  geom_curve(data = ref_data %>% filter(parameter == "HIS_451"),
             aes(xend = 22.75,
                 yend = value + 0.1,
                 x = 23.5,
                 y = value + 1.25),
  ...
  geom_text(data = ref_data %>% filter(parameter == "HIS_451"),
             aes(x = 23.5,
                 y = value + 1.35),
            label = "ZmLAC3",
            hjust = 0,
  ...
  theme_leo() +
  theme(legend.position = c(0.83, 1.025),
        legend.direction = "horizontal",
        legend.key.height = unit(2, "mm"),
        plot.margin = unit(c(7.5, 1, 1, 1), "mm"),
        axis.title = element_blank(),
        strip.placement = "outside",
        strip.text = ggtext::element_markdown(hjust = 0.5, face = "plain", margin = unit(c(1, 1, 1, 1), "mm"))) +
  coord_flip(clip = "off", xlim = c(1, 22)) +
