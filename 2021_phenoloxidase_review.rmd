...
```{r}
LAC <- read_csv("/home/leonard/Dropbox/2018_PO_review/2020_rewrite/Data/sequence_conservation/LAC_conservation.csv",
                col_names = c("PO", 1:550)) %>% 
  mutate(PO = "LAC")

PRX <- read_csv("/home/leonard/Dropbox/2018_PO_review/2020_rewrite/Data/sequence_conservation/PRX_conservation.csv",
                col_names = c("PO", 1:314)) %>% 
  mutate(PO = "PRX")

PPO <- read_csv("/home/leonard/Dropbox/2018_PO_review/2020_rewrite/Data/sequence_conservation/PPO_conservation.csv",
                col_names = c("PO", 1:339)) %>% 
  mutate(PO = "PPO")

conservation <- bind_rows(LAC, PRX, PPO) %>% 
  pivot_longer(-PO, names_to = "position", values_to = "conservation") %>% 
  mutate(position = as.numeric(position),
         PO = ordered(PO, levels = c("PPO", "LAC", "PRX"))) %>% 
  drop_na()

conservation_count <- conservation %>% 
  mutate(conservation = round(conservation, digits = 0)) %>% 
  uncount(conservation)
```
...
conservation_plot <- ggplot(conservation_count,
       aes(x = position,
           fill = PO)) +
  geom_histogram(stat = "count", width = 1.1) +
  scale_fill_few() +
  theme_void() +
  coord_fixed() + 
  facet_wrap(~ PO, ncol = 1)

pdf("conservation_histogram.pdf")
conservation_plot
dev.off()
```
...
po_plot <- ggplot(data = PO, aes(x = genome, y = value)) +
  ...
  scale_size_area(max_size = 5, name = "Divergence [mya]", breaks = c(10, 100, 1000)) +
...
pocket_residues <- read_csv("/home/leonard/Dropbox/2018_PO_review/2020_rewrite/Modelling/pocket_residues.csv") %>%
  ...
  mutate(
    ...
    position = case_when(
      paralog == "LcLAC" & position > 323 ~ position - 1,
      TRUE ~ position
    )
  )
...
CASTp_poc_id <- CASTp_poc %>%
  ...
  filter(contact_residues == max(contact_residues)) %>%
  # select only the pocket with contact to the maximum number of pocket residues
  filter(contact_atoms == max(contact_atoms)) %>%
  # if still > 1 pocket, select the one with contact to maximum atoms of pocket residues
...
zmlac3_pocInfo <- map_dfr(
  c(
    "/home/leonard/Dropbox/2018_PO_review/2020_rewrite/Modelling/CASTp/ZmLAC3/6klg.pocInfo",
    "/home/leonard/Dropbox/2018_PO_review/2020_rewrite/Modelling/CASTp/ZmLAC3/6klj.pocInfo"
  ),
  read_pocInfo
) %>%
  mutate(
...
mouthInfo <- map(mouthInfo_files, read_mouthInfo) %>%
  bind_rows() %>%
  select(Molecule, ID, "mouth_area_sa" = Area_sa, "mouth_diameter" = Len_sa)

zmlac3_mouthInfo <- map_dfr(
  c(
    "/home/leonard/Dropbox/2018_PO_review/2020_rewrite/Modelling/CASTp/ZmLAC3/6klg.mouthInfo",
    "/home/leonard/Dropbox/2018_PO_review/2020_rewrite/Modelling/CASTp/ZmLAC3/6klj.mouthInfo"
  ),
  read_mouthInfo
) %>%
  mutate(
...
ref_pocket <- zmlac3_pocInfo %>%
  left_join(select(zmlac3_mouthInfo, paralog, model, mouth_area_sa),
    by = c("paralog", "model")
  ) %>%
  filter(model == "6klg") %>%
  mutate(
...
pocket_data <- pocInfo %>%
  ...
  filter(ID == consensus_pocket) %>%
  filter(!(str_detect(model, "IL"))) %>%
  # remove low_quality initial models
  mutate(
    compactness = ((pi^(1 / 3)) * ((6 * `Vol_sa`)^(2 / 3))) / `Area_sa`
  )
...
vol <- ggplot(
  ...
  geom_hline(
    data = zmlac3_pocInfo[1, ],
    aes(yintercept = Vol_sa),
    size = 0.1
  ) +
  geom_hline(
    data = zmlac3_pocInfo[2, ],
    aes(yintercept = Vol_sa),
    size = 0.1,
    linetype = 3
  ) +
...
for (i in 1:nrow(best_models)) {
  ...
  sphere <- tibble(sphere[[pocket]]) %>%
    # select correct pocket
    mutate(
      "RecordName" = "ATOM",
      "Serial" = case_when(
        row_number() < 10 ~ paste0("     ", row_number()),
      ...
      "ResidueName" = "STP",
      # "ChainID" = "A",
      "ResidueNumber" = case_when(
        pocket < 10 ~ paste0("    ", pocket, "    "),
...
pocket <- pocket_atm %>%
  ...
  # if still > 1 pocket, select smallest pocket
  mutate(
...
his_depth <- depth %>%
  ...
  mutate(
    ...
    residue = case_when(
      paralog == "LcLAC" & model == "relaxed" & residue > 323 ~ residue + 1,
      TRUE ~ residue
    )
  ) %>%
  filter(!(str_detect(model, "IL"))) %>%
...
corrplot::corrplot(cor(corr_data),
                   # method = "number",
                   addCoef.col = "black",
                   type = "upper"
                   )
pdf("supp_cluster.pdf", height = 4, width = 4)
PerformanceAnalytics::chart.Correlation(corr_data)
dev.off()
...
model_data_modeller <- model_data_modeller %>%
  mutate(paralog = ordered(paralog, levels = gg_clust[["labels"]][["label"]]))
...
violins_modeller <- ggplot(
  ...
  geom_quasirandom(
    data = model_data_modeller %>% filter(model != "relaxed"),
    aes(fill = DOPE, group = model),
    shape = 21,
    width = 0.2,
    alpha = 1,
    size = 1,
  ...
  #   arrow = arrow(length = unit(0.03, "npc"))
  # ) +
  scale_fill_gradientn(colours = rev(morgenstemning), limits = c(-2.1, -0.4)) +
  expand_limits(y = 0) +
  labs(fill = "Model quality\n(DOPE Z-score)") +
  theme_leo() +
  theme(
...
```{r}
exp_data <- model_data_modeller %>%
  bind_rows(model_data_fungus, ref_data) %>%
  mutate(
    kingdom = case_when(
      is.na(kingdom) ~ "plantae",
      TRUE ~ kingdom
    ),
    model = case_when(
      paralog == "1KYA (X-ray)" ~ "crystallography",
      model == "relaxed" ~ "AlphaFold 2",
      model == "6klg" ~ "crystallography",
      TRUE ~ model
    ),
    DOPE = case_when(
      paralog == "ZmLAC3" ~ -1.740,
      TRUE ~ DOPE
    )
  ) %>%
  pivot_wider(names_from = parameter, values_from = value)

write_csv(exp_data, "model_data.csv")
```
...
phylogeny_species <- read_delim("/home/leonard/Dropbox/2018_PO_review/2020_rewrite/Phylogeny/2020-04-14_mrbayes/flat_prior/removed_X/no_Beagle/species.txt",
  ...
  mutate(
    ...
    species = case_when(
      ...
      str_detect(species, fixed("Physcomitrella")) ~ "[Physcomitrium patens]",
      TRUE ~ species
    ),
    ...
    #   TRUE ~ species
    # ),
    species = ordered(species, levels = c(
      "Chlamydomonas reinhardtii",
      "Volvox carteri",
      "Chara braunii",
    ...
    species_colour = droplevels(replace(species, species %in% c(
      "Zea mays",
      "Chlamydomonas reinhardtii",
      "Miscanthus sinensis"
    ), NA))
