...
### [unused] Stem thickness and count
...
thickness_data <- pheno_data %>% 
  ...
  mutate(
    ...
    `thickness/height` = thickness / stretched.height) %>% 
  pivot_longer(c(thickness, stems, `thickness/height`), names_to = "variable", values_to = "value")
...
```{r}
pdf("lodging.pdf", width = twocol, height = onecol)
base_delta + base_delta_grouped + straightness_plot + straightness_grouped +
...
rmn_maule_plot <- ggplot(rmn_maule,
  ...
  geom_tile(
    ...
    aes(fill = colour,
        ...
        width = 0.1,
        y = hue),
    position = position_nudge(y = 0, x = 1.975)
  ) +
  geom_text(data = bar_data %>% filter(colour == 9),
            position = position_nudge(y = 5, x = 2.075),
            label = "Visual hue scale",
            angle = -90,
            hjust = 0,
            family = "Helvetica",
  ...
  scale_y_continuous(limits = c(324, 416), 
                     ...
                     breaks = c(340, 360, 380, 400),
                     labels = c("340", "360", "20", "40")) +
  scale_color_manual(values = pal_dawn) +
...
pdf("maule_SG.pdf", height = onecol * 0.5, width = onecol * 0.8)
rmn_maule_plot
dev.off()
```
