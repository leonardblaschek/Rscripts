...
scheme_genes <- read_csv("/home/leonard/Dropbox/2021_lac_manuscript/Expression/scheme_features.csv") %>% 
  group_by(gene) %>% 
  mutate(beginning = min(c(start, stop)),
         end = max(c(start, stop)))
...
gene_scheme <- ggplot(
  ...
  scale_fill_manual(values = c(lighten(pal_ostwald_disc[1], 0.2), pal_ostwald_disc[3], "grey80")) +
  theme_leo() +
...
pdf("gene_models.pdf", width = twocol, height = onecol * 0.3)
gene_scheme
dev.off()
```
