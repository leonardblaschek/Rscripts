...
read_CC <- function(flnm) {
  read_csv(flnm) %>%
    ...
    select(-notes) %>% 
    pivot_longer(-c("date", "genotype", "replicate", "speed", "sum"), names_to = "ROI", values_to = "value") %>%
...
CC_plot <- ggplot(
  ...
  #   family = "Helvetica"
  # ) +
  labs(y = "UV autofluorescence") +
  scale_fill_manual(values = pal_dawn[c(1, 5, 7)]) +
  # scale_y_continuous(breaks = c(0.5, 0.75, 1)) +
  # scale_x_discrete(limits = c(0, 1)) +
  # scale_shape_manual(values = c(21, 22, 23, 24, 25)) +
  theme_leo() +
