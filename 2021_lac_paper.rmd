...
rmn_maule_plot <- ggplot(rmn_maule,
  ...
  geom_tile(
    ...
    aes(fill = colour,
        ...
        width = 0.05,
        y = hue),
    position = position_nudge(y = 0, x = 1.92)
  ) +
  geom_text(data = bar_data %>% filter(colour == 9),
            position = position_nudge(y = 5, x = 1.96),
            label = "Visual hue scale",
            angle = -90,
            hjust = 0,
            family = "Helvetica",
...
pdf("maule_SG.pdf", height = onecol * 0.5, width = twocol)
rmn_maule_plot
dev.off()
```
...
pdf("Blaschek2022_figureS4.pdf", height = onecol * 1.5, width = twocol)
maule_hue_plot + maule_OD_plot + rmn_maule_plot +
  plot_layout(design = layout) &
  plot_annotation(
    tag_levels = "A"
  ) &
...
pyro_data <- read_csv("/home/leonard/Dropbox/2021_lac_manuscript/Biochemistry/pyrolysis.csv") %>%
  ...
  mutate(
    ...
    "**G**<sub>CHOH</sub> [% of lignin]" = GCHO / lignin_raw * 100,
    "**S**<sub>CHO</sub> [% of lignin]" = SCHO / lignin_raw * 100,
    "**S**<sub>CHOH</sub> [% of lignin]" = SCHO / lignin_raw * 100,
...
writexl::write_xlsx(growth_data, "TE_differentiation.xlsx")
```
...
EEEE
"
pdf("Blaschek2022_figureS1.pdf", width = twocol, height = twocol)
