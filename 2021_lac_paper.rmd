...
Wiesner_plot <- ggplot(
  ...
  labs(y = "Relative coniferaldehyde") +
  scale_color_manual(values = pal_dawn, aesthetics = c("colour", "fill")) +
  # scale_x_discrete(limits = c(0, 1)) +
  # scale_shape_manual(values = c(21, 22, 23, 24, 25)) +
  theme_leo() +
...
data_dab <- data_dab %>%
  filter(
    cell_type %in% c("IF", "CML", "XF", "MX", "PX") &
      pH == 5 &
      treatment == "none"
    # adj.r.squared > 0.85
  )
...
ACCCCCC
"
pdf("section_activity.pdf", width = twocol, height = onecol)
...
violin_data <- rmn_lac_sum %>%
  select(
    # "Total lignin" = total_lignin,
    "<sup>Lignin</sup>&frasl;<sub>cellulose</sub>" = lig.cellu,
...
pdf("Raman_fig.pdf", width = twocol, height = onecol)
lig_violin
dev.off()
```
