...
#function to print zero without decimals
clean_zero <- function(l){
    max.decimals <- max(c(nchar(str_extract(l, "\\.[0-9]+")), 0), na.rm = T)
    if (max.decimals > 0) {max.decimals <- max.decimals - 1}
...
stat_path <- "/home/leonard/Dropbox/2021_lac_manuscript/Submission/PlantCell_revised - published/supplementary/statistics/accepted/"
...
datapath <- "/run/media/leonard/data/grsync/data/"
```
...
growth_files <-
  list.files(
    path = paste0(datapath, "PhD/2021-01_inductions/"),
    pattern = "*.txt",
    recursive = FALSE,
    full.names = TRUE
  )
...
growth_data <- map(growth_files, read_plus) %>%
  ...
  mutate(
    ...
    comment = case_when(is.na(code) ~ "viability estimated",
                        TRUE ~ "viability quantified")
  )
...
growth_facet <- ggplot(
  ...
  geom_rect(
    data = tibble(variable = "TE viability", dpi = 1, value = 1),
    fill = "grey80",
    xmin = 4.8,
    xmax = Inf,
    ymin = -Inf,
  ...
  geom_text(
    data = tibble(variable = "TE viability", dpi = c(0, 15), value = 0.5, label = c("TEs alive", "TEs dead"), just = c(0, 1)),
    aes(label = label,
        hjust = just),
    family = "Helvetica",
    size = ggtext_size,
...
pdf("TE_growth_sigmoid.pdf", width = twocol, height = onecol * 0.4)
growth_facet
dev.off()
```
...
```{r}
pyro_TE_data <- read_csv("/home/leonard/Dropbox/2021_lac_manuscript/Biochemistry/pyrolysis.csv") %>%
  filter(sample %in% c("WT", "TEs", "Parenchyma")) %>%
  mutate(sample = recode(sample, "Parenchyma" = "PA")) %>% 
  mutate(
    lignin_raw = H + C + G + `5H` + S + P,
    "Lignin [% of CW]" = case_when(sample == "TEs" ~ (lignin_raw / SUM * 100) / 0.4,
                                                 TRUE ~ lignin_raw / SUM * 100),
    "G [% of lignin]" = G / lignin_raw * 100,
    "S [% of lignin]" = S / lignin_raw * 100,
    "H [% of lignin]" = H / lignin_raw * 100,
    "5H [% of lignin]" = `5H` / lignin_raw * 100,
    "G<sub>CHO</sub> [% of lignin]" = GCHO / lignin_raw * 100,
    "G<sub>CHOH</sub> [% of lignin]" = GCHOH / lignin_raw * 100,
    "S<sub>CHO</sub> [% of lignin]" = SCHO / lignin_raw * 100,
    "S<sub>CHOH</sub> [% of lignin]" = SCHOH / lignin_raw * 100,
    # "<sup>H</sup>&frasl;<sub>G</sub>" = H / G,
    # "<sup>5H</sup>&frasl;<sub>G</sub>" = `5H` / G,
    "<sup>S</sup>&frasl;<sub>G</sub>" = S / G,
    "<sup>G<sub>CHO&thinsp;</sub></sup>&frasl;<sub>G<sub>CHOH</sub></sub>" = GCHO / GCHOH,
    "<sup>S<sub>CHO&thinsp;</sub></sup>&frasl;<sub>S<sub>CHOH</sub></sub>" = SCHO / SCHOH,
    ...
    type = ordered(type, levels = c("hypocotyl", "culture"))
  )  %>%
  # rename("H" = H,
  #        "C" = C,
  #        "G" = G,
  ...
  #        "S" = S,
  #        "P" = P) %>%
  # select(-c(SUM, lignin_raw, GCHO, SCHO, GCHOH, SCHOH)) %>%
  select(-c(5:16)) %>%
  pivot_longer(!c(type, sample, replicate, method), names_to = "variable", values_to = "value") %>%
  mutate(variable = ordered(variable, levels = c(
    ...
    "H [% of lignin]",
    "5H [% of lignin]",
    "G<sub>CHO</sub> [% of lignin]",
    "G<sub>CHOH</sub> [% of lignin]",
    "S<sub>CHO</sub> [% of lignin]",
    "S<sub>CHOH</sub> [% of lignin]",
    "<sup>S</sup>&frasl;<sub>G</sub>",
    # "<sup>H</sup>&frasl;<sub>G</sub>",
    # "<sup>5H</sup>&frasl;<sub>G</sub>",
    "<sup>G<sub>CHO&thinsp;</sub></sup>&frasl;<sub>G<sub>CHOH</sub></sub>",
    "<sup>S<sub>CHO&thinsp;</sub></sup>&frasl;<sub>S<sub>CHOH</sub></sub>"
  )))

pyro_TE_avg <- pyro_TE_data %>%
  group_by(type, sample, method, variable) %>%
...
```{r}
letters <- letter_groups(pyro_TE_data,
                         value,
                         sample,
                         "tukey",
                         variable,
...
pyro_TE_plot <- ggplot(pyro_TE_data,
                    aes(x = sample,
                        y = value)) +
  geom_col(data = pyro_TE_avg,
           aes(y = mean,
               fill = type)) +
  geom_quasirandom(
    shape = 16,
  ...
  scale_fill_manual(values = c(pal_dawn[1], "grey80", "grey80"), name = "Sample") +
  scale_y_continuous(labels = clean_zero) +
  theme_leo() +
  theme(strip.text.y.left = element_textbox_simple(halign = 0.5, 
...
pdf("biochem_TE_supp.pdf", width = twocol, height = onecol)
pyro_TE_plot
dev.off()
```
...
```{r}
stat_tab <- pyro_TE_data %>%
  drop_na(value) %>% 
  rowwise() %>%
  mutate(sample = strip_html(sample),
...
## [unused] Expression as binary time points

```{r, eval=FALSE}
expression_short <- read_csv("/run/media/leonard/data/grsync/Documents/Uni/Master/Master thesis/Laccases/Expression/R_file.csv") %>%
  mutate(
    time = case_when(
...
```{r}
array <- read_csv("/home/leonard/Dropbox/2021_lac_manuscript/Expression/microarray.csv") %>% 
  pivot_longer(-c(type, gene, locus), names_to = "rep_time", values_to = "expression") %>% 
  separate(rep_time, into = c("replicate", "time")) %>% 
  mutate(day = as.numeric(str_remove(time, "h")) / 24,
         method = "Microarray",
         sample = "Induced")
```
...
```{r}
qPCR <- read_csv("/home/leonard/Dropbox/2021_lac_manuscript/Expression/qPCR.csv") %>% 
  fill(sample, gene) %>% 
  left_join(array %>% select(gene, type)) %>% 
  mutate(type = case_when(is.na(type) ~ "reference",
                          TRUE ~ type),
         method = "RT-qPCR") %>% 
  pivot_longer(-c(method, type, sample, gene, day), names_to = "replicate", values_to = "expression")
```
...
expression <- array %>% 
  bind_rows(qPCR) %>% 
  select(-c(locus, time)) %>% 
  mutate(type = case_when(type %in% c("SCW", "PCD") ~ paste(type, "marker"),
                          TRUE ~ type))

expression_avg <- expression %>% 
  group_by(sample, type, gene, day, method) %>% 
  ...
  mutate(gene = ordered(gene, levels = c(
    "CesA7",
    "XCP2",
    "CCR1",
    "F5H1",
...
```{r}
array_profiles <- ggplot(
  expression_avg %>% filter(method == "Microarray"),
  aes(
    x = day,
    y = mean
  )
  ...
  labs(x = "Days past induction",
       y = "Relative expression [microarray]") +
  theme_leo() +
...
pdf("array_profiles.pdf", width = twocol, height = onecol)
array_profiles
dev.off()
```
...
```{r}
qPCR_profiles <- ggplot(
  expression_avg %>% filter(method == "RT-qPCR"),
  aes(
    x = day,
    y = mean
  )
  ...
  guide = "none") +
  scale_y_continuous(labels = clean_zero) +
  scale_x_continuous(labels = clean_zero) +
  labs(x = "Days past induction",
       y = "Relative expression<br>[RT-qPCR &#8710;Ct]") +
  theme_leo() +
  theme(
    strip.placement = "outside",
...
pdf("qPCR_profiles.pdf", width = twocol, height = onecol)
qPCR_profiles
dev.off()
```
...
```{r}
corr_data <- expression_avg %>% 
  filter(method == "Microarray") %>% 
  ungroup() %>% 
  select(gene, day, mean) %>% 
  pivot_wider(names_from = gene, values_from = mean) %>% 
  column_to_rownames("day")

corr_matrix <- cor(corr_data)

corr_tibble <- corr_matrix %>% 
  as_tibble(rownames = "gene1") %>% 
  pivot_longer(-gene1, names_to = "gene2", values_to = "correlation") %>% 
  mutate(gene1 = ordered(gene1, levels = levels(expression_avg$gene)),
         gene2 = ordered(gene2, levels = levels(expression_avg$gene))) %>% 
  filter(gene2 %in% c("CesA7", "XCP2"))

corr_plot <- ggplot(corr_tibble,
       aes(x = gene1,
           y = gene2)) +
  geom_tile(aes(fill = correlation),
            colour = "white") +
  # scale_fill_gradientn(colours = rev(pal_ostwald_cont), name = "Pearson's <i>r</i>") +
...
EEEE
"
pdf("Blaschek2022_figure1.pdf", width = twocol, height = twocol)
growth_facet +  pyro_TE_plot + array_profiles + corr_plot + qPCR_profiles +
  plot_layout(
    design = layout
    # heights = c(0.75, 2, 0.4, 1.5, 1.5)
    # design = layout
    ) &
  plot_annotation(
    tag_levels = list(LETTERS[2:6])
  ) &
...
pheno_full <- bind_rows(pheno_2017, pheno_2020) %>%
  ...
  mutate(
    ...
    genotype = case_when(
      ...
      TRUE ~ paste0("<i>", str_replace(genotype, "Q-", "Q−"), "</i>")
    ),
...
siliques <- read_csv(paste0(
  ...
  mutate(genotype = ordered(case_when(
    genotype == "WT" ~ genotype,
    TRUE ~ paste0("<i>", str_replace(genotype, "-", "−"), "</i>")
  ),
...
stat_tab <- pheno_data %>%
  ...
  anova_tab <- stat_tab$anova %>% 
    ...
    distinct()
  write_csv(anova_tab, paste0(stat_path, "02G_anova.csv"))

  tukey_tab <- stat_tab$tukey %>%
    add_column("variable" = "height [cm]") %>% 
    distinct()
  write_csv(tukey_tab, paste0(stat_path, "02G_tukey.csv"))
```
...
stat_tab <- siliques_avg %>% filter(genotype %in% c(WT, quad_it, Q_it)) %>%
  ...
  anova_tab <- stat_tab$anova %>% 
    ...
    distinct()
  write_csv(anova_tab, paste0(stat_path, "S02G_anova.csv"))

  tukey_tab <- stat_tab$tukey %>%
    add_column("variable" = "silique length [cm]") %>% 
    distinct()
  write_csv(tukey_tab, paste0(stat_path, "S02G_tukey.csv"))
```
...
thickness_data <- pheno_data %>% 
  ...
  mutate(
    ...
    T.H = thickness / stretched.height)
```

#### Thickness

```{r}
letters <- letter_groups(thickness_data,
  thickness,
  genotype,
  "tukey",
  print_position = "below",
...
pdf("thickness.pdf", height = onecol * 0.5, width = onecol * 1.5)
thickness
dev.off()
```
...
stat_tab <- thickness_data %>%
  ...
  anova_tab <- stat_tab$anova %>% 
    add_column("variable" = "stem diameter [cm]") %>% 
    distinct()
...
thickness.height <- ggplot(
  ...
  labs(y = "Stem <sup>diameter</sup>/<sub>height</sub>") +
  scale_color_manual(values = pal_dawn, aesthetics = c("colour", "fill")) +
  # scale_x_discrete(limits = c(0, 1)) +
  # scale_shape_manual(values = c(21, 22, 23, 24, 25)) +
  theme_leo() +
...
pdf("T_H.pdf", height = onecol * 0.5, width = onecol * 1.5)
thickness.height
dev.off()
```
...
stat_tab <- thickness_data %>%
  ...
  anova_tab <- stat_tab$anova %>% 
    add_column("variable" = "stem diameter [cm] / stem height [cm]") %>% 
    distinct()
...
pdf("stem_count.pdf", height = onecol * 0.5, width = onecol * 1.5)
stem_no
dev.off()
```
...
stat_tab <- thickness_data %>%
  ...
  anova_tab <- stat_tab$anova %>% 
    add_column("variable" = "stem count") %>% 
    distinct()
...
stat_tab <- flower_data %>%
  ...
  anova_tab <- stat_tab$anova %>% 
    ...
    distinct()
  write_csv(anova_tab, paste0(stat_path, "S02F_anova.csv"))

  tukey_tab <- stat_tab$tukey %>%
    add_column("variable" = "bolting [dpg]") %>% 
    distinct()
  write_csv(tukey_tab, paste0(stat_path, "S02F_tukey.csv"))
```
...
stat_tab <- nested_stem_data %>%
  ...
  anova_tab <- stat_tab$anova %>% 
    ...
    distinct()
  write_csv(anova_tab, paste0(stat_path, "02F_anova.csv"))

  tukey_tab <- stat_tab$tukey %>%
    add_column("variable" = "stem growth rate [cm/day]") %>% 
    distinct()
  write_csv(tukey_tab, paste0(stat_path, "02F_tukey.csv"))
```
...
ID <- read_csv(paste0(
  ...
  add_row(read_csv(paste0(
    datapath,
    "PhD/Phenotyping/2021-04_lac_mutants/plant_order.csv"
  )) %>% mutate(instance = "2021-04")) %>%
  select(genotype, replicate, plant, instance) %>%
  mutate(plant = as.character(plant))

scales <- read_csv(paste0(
  datapath,
  "PhD/Phenotyping/2020-09_lac_mutants/size_marker.csv"
),
col_names = c("filename", "space")
  ...
  mutate(image = str_remove(filename, fixed(".jpg"))) %>%
  separate(image, into = c("dpg", "plant")) %>%
  select(plant, instance, dpg, space) %>% 
  mutate(dpg = as.numeric(dpg)) %>%
  distinct()
...
angles <- read_csv("/home/leonard/Dropbox/2021_lac_manuscript/lodging/stem_angles.csv") %>%
  ...
  left_join(scales) %>% 
  mutate(
    stem_length = stem_length / space * 1.5,
    bounding_box_height = bounding_box_height / space * 1.5,
    straightness = bounding_box_height / stem_length,
  ...
  group_by(genotype) %>% 
  mutate(size_cat = case_when(stem_length > median(stem_length, na.rm = TRUE) ~ "Longer than genotype-median",
                         TRUE ~ "Shorter than genotype-median"),)
```
...
stat_tab <- angles %>%
  ...
  anova_tab <- stat_tab$kruskal %>% 
    add_column("variable" = "stem base angle [∆90°]") %>% 
    distinct()
  write_csv(anova_tab, paste0(stat_path, "S02K_kruskal.csv"))

  tukey_tab <- stat_tab$dunn %>%
    add_column("variable" = "stem base angle [∆90°]") %>% 
    distinct()
  write_csv(tukey_tab, paste0(stat_path, "02K_dunn.csv"))
```

#### [unused] Plot straightness
...
#### [unused] Correlations with height
...
lodging_regression <- lm(base_d90 ~ stem_length + as.character(genotype),
   data = angles)

summary(lodging_regression)

plot(lodging_regression)
```
...
stat_tab <- nested_root_data %>%
  ...
  anova_tab <- stat_tab$anova %>% 
    ...
    distinct()
  write_csv(anova_tab, paste0(stat_path, "S02E_anova.csv"))

  tukey_tab <- stat_tab$tukey %>%
    add_column("variable" = "root growth rate [cm/day]") %>% 
    distinct()
  write_csv(tukey_tab, paste0(stat_path, "S02E_tukey.csv"))
```
...
```{r}
pdf("Blaschek2022_figure2.pdf", height = onecol * 0.8, width = twocol)
area_plot +
  inset_element(rosette_growth_plot,
    left = 0.10, bottom = 0.55, right = 0.65, top = 0.85,
    align_to = "full"
  ) +
  height_plot + 
  inset_element(stem_growth_plot,
    left = 0.02, bottom = 0.675, right = 0.53, top = 0.975,
    align_to = "full"
  ) +
  inset_element(final_height,
    left = 0.42, bottom = 0.08, right = 0.93, top = 0.38,
    align_to = "full"
  ) 
dev.off()
```
...
pdf("Blaschek2022_figureS2.pdf", height = twocol, width = twocol)
full_mutant_plot + 
  thickness + thickness.height + stem_no +
  roots + bolt_geno + silique_plot + 
  base_delta + 
  plot_layout(design = layout) &
  plot_annotation(
    tag_levels = list(c("A", "B", "C", "D", "E", "F", "G", "K"))
  ) &
  theme(plot.tag = element_text(size = 10, face = "bold", family = "Helvetica"))
dev.off()
```
...
# include autoclaved DAB data
dab_autoclaved <- read_csv(c(
  paste0(
    datapath,
    "PhD/Laccase_activity/2020-09_activity/2020-09_lac_activity.csv"
  ),
  paste0(
    datapath,
    "PhD/Laccase_activity/2020-06_activity/DAB/DAB_CC/Measurements/2020-07-07_WT_autoclaved_DAB_CC.csv"
  )
),
comment = '"#'
) %>%
...
activity_opt_files <-
  list.files(
    path = c(paste0(datapath, "PhD/Laccase_activity/2021-01_optimal_pH/ROIs/Measurements/"),
             paste0(datapath, "PhD/Laccase_activity/2021-01_optimal_pH/DAF_CC_ROIs/Measurements/"),
...
autoclaved_slopes <- avg_pH_slopes %>% 
  filter(treatment == "autoclaved" &
           genotype == "WT" &
           pH == 5 &
           cell_type != "LP") %>% 
  group_by(substrate, pH, cell_type) %>% 
  summarise(slope_autoclaved = mean(mean_estimate, na.rm = TRUE),
            sd_autoclaved = mean(sd_estimate))
```
...
pygl_violin <- ggplot(
  ...
  scale_y_continuous(limits = c(0, 0.002), labels = clean_zero) +
  theme_leo() +
  theme(
    # axis.title.x = element_blank(),
...
DAF_violin <- ggplot(
  ...
  labs(y = "DAF oxidation rate") +
  scale_color_manual(values = c(pal_dawn[1], "#E3E6E8", pal_dawn[7], "#F3EAD8"), aesthetics = c("colour", "fill")) +
  scale_y_continuous(limits = c(0, 0.0037), labels = clean_zero) +
  theme_leo() +
  theme(
    # axis.title.x = element_blank(),
    # axis.text.x = element_markdown(angle = 45, hjust = 1, vjust = 1),
...
dab_slopes <- ggplot(
  ...
  geom_hline(data = autoclaved_slopes %>% filter(substrate == "DAB"),
            aes(yintercept = slope_autoclaved),
            size = 0.2,
            linetype = 2) +
  geom_quasirandom(
    shape = 16,
  ...
  labs(y = "DAB oxidation rate<br>at pH 5") +
  scale_color_manual(values = pal_dawn, aesthetics = c("colour", "fill")) +
  scale_y_continuous(labels = clean_zero) +
  theme_leo() +
  theme(
...
stat_tab <- avg_slopes_dab %>%
  ...
  anova_tab <- stat_tab$anova %>% 
    add_column(
      ...
      "cell_type" = stat_tab$cell_type) %>% 
    distinct()
  write_csv(anova_tab, paste0(stat_path, "03C_anova.csv"))
  ...
  tukey_tab <- stat_tab$tukey %>%
    add_column(
      ...
      "cell_type" = stat_tab$cell_type) %>% 
    distinct()
  write_csv(tukey_tab, paste0(stat_path, "03C_tukey.csv"))
```
...
#   group_by(date, substrate, treatment, pH, cell_type) %>%
#   mutate(rel_estimate = estimate / mean_estimate[genotype == "WT"])
 test<- nested_data_daf %>% 
   filter(genotype == "<i>Q−4</i>", replicate == "1", substrate == "DAF", treatment == "none", pH == "5", date == "2020-08-27", cell_type == "IF")
...
daf_slopes <- ggplot(
  ...
  geom_hline(data = autoclaved_slopes %>% filter(substrate == "DAF"),
            aes(yintercept = slope_autoclaved),
            size = 0.2,
            linetype = 2) +
  geom_quasirandom(
  ...
  labs(y = "DAF oxidation rate<br>at pH 5") +
  scale_color_manual(values = pal_dawn, aesthetics = c("colour", "fill")) +
  scale_y_continuous(labels = clean_zero) +
  theme_leo() +
  theme(
...
ACCCCCC
"
pdf("Blaschek2022_figure3.pdf", width = twocol, height = onecol * 1.2)
...
```{r}
maule_hue <- read_csv("/home/leonard/Dropbox/2021_lac_manuscript/Mäule/maule_hue.csv") %>%
  mutate(
    hue = hue255 / 255 * 360,
    hue = case_when(
      hue < 100 ~ hue + 360,
...
maule_OD <- read_csv("/home/leonard/Dropbox/2021_lac_manuscript/Mäule/maule_OD.csv") %>%
  mutate(
    cell_type = recode(cell_type,
                       "CML" = "IF-CML",
                       "CC" = "IF-CC"),
    PH_OD = median(OD[cell_type == "PH"]),
    cell_type = ordered(cell_type,
      levels = c(
        "IF",
        "IF-CML",
...
hue_palette1 <- hue_pal(h = c(330, 360) + 20, c = 200, l = 45, direction = 1)
hue_palette2 <- hue_pal(h = c(10, 60) + 20, c = 200, l = 45, direction = 1)
hue_palette <- c(hue_palette1(n = 4), hue_palette2(n = 5))
show_col(hue_palette)
...
## Mäule absorbance violins

```{r}
maule_OD_plot <- ggplot(
  maule_OD,
  aes(
    x = genotype,
    y = OD
  ...
  geom_hline(aes(yintercept = PH_OD),
             linetype = 2,
             size = 0.2) +
  geom_quasirandom(
    shape = 16,
  ...
  scale_fill_manual(values = pal_dawn) +
  scale_y_continuous(limits = c(0, 0.5), labels = clean_zero) +
  labs(y = "Mäule stain absorbance") +
  theme_leo() +
  theme(
    axis.title.x = element_blank(),
    axis.title.y = element_textbox_simple(
...
pdf("maule_plot_OD.pdf", width = twocol, height = onecol * 0.4)
maule_OD_plot
dev.off()
```
  ...
  summarise(
    ...
    "broad_lignin" =
      corrected.intensity[abs(wavenumber - 1600) == min(abs(wavenumber - 1600))][1] +
        corrected.intensity[abs(wavenumber - 1334) == min(abs(wavenumber - 1334))][1] +
        corrected.intensity[abs(wavenumber - 1658) == min(abs(wavenumber - 1658))][1] +
        corrected.intensity[abs(wavenumber - 1634) == min(abs(wavenumber - 1634))][1],
    "total_lignin" = (scaled[abs(wavenumber - 1600) == min(abs(wavenumber - 1600))][1] / 2) +
      scaled[abs(wavenumber - 1334) == min(abs(wavenumber - 1334))][1],
    "P_rel" = scaled_lig[abs(wavenumber - 993) == min(abs(wavenumber - 993))][1],
    "G_rel" = scaled_lig[abs(wavenumber - 1600) == min(abs(wavenumber - 1600))][1],
    "G_tot" = scaled[abs(wavenumber - 1600) == min(abs(wavenumber - 1600))][1],
    "GOH_rel" = scaled_lig[abs(wavenumber - 1658) == min(abs(wavenumber - 1658))][1],
    "GCHO_rel" = scaled_lig[abs(wavenumber - 1634) == min(abs(wavenumber - 1634))][1],
    "GCHO_tot" = scaled[abs(wavenumber - 1634) == min(abs(wavenumber - 1634))][1],
    "S_rel" = scaled_lig[abs(wavenumber - 1334) == min(abs(wavenumber - 1334))][1],
    "S_tot" = scaled[abs(wavenumber - 1334) == min(abs(wavenumber - 1334))][1],
    "cellulose" =
      # scaled[abs(wavenumber - 1095) == min(abs(wavenumber - 1095))][1] +
    ...
    "GOH.G" = scaled_lig[abs(wavenumber - 1658) == min(abs(wavenumber - 1658))][1] /
      scaled_lig[abs(wavenumber - 1600) == min(abs(wavenumber - 1600))][1],
    "GCHO.GOH" = scaled_lig[abs(wavenumber - 1634) == min(abs(wavenumber - 1634))][1] /
      scaled_lig[abs(wavenumber - 1658) == min(abs(wavenumber - 1658))][1],
    "O4_van_rel" = scaled_lig[abs(wavenumber - 1303) == min(abs(wavenumber - 1303))][1],
    "other_van_rel" = scaled_lig[abs(wavenumber - 785) == min(abs(wavenumber - 785))][1],
    "O4_syr_rel" = scaled_lig[abs(wavenumber - 1297) == min(abs(wavenumber - 1297))][1],
...
rmn_ref_n <- rmn_ref_sum %>% 
  group_by(genotype, cell_type)

rmn_ref_avg <- rmn_ref_sum %>% 
  group_by(genotype, cell_type) %>% 
  summarise(across(where(is.numeric), ~ mean(.x, na.rm = TRUE)))
```
...
partial_spec <- function(min_wn, max_wn) {
  ...
  ggplot(
    ...
    scale_y_continuous(limits = c(-0.1, 8), labels = clean_zero) +
...
partial_spec <- function(min_wn, max_wn) {
  ...
  ggplot(
    ...
    geom_vline(
      xintercept = c(378, 1273, 1334, 1600, 1634, 1658),
      linetype = 2,
      size = 0.2
    ) +
...
ref_labels <- tribble(
  ...
  "fah1", "MX", "<sup>S</sup>&frasl;<sub>G</sub>", "<i>fah1</i> MX (traces of S)",
  "ccr1_3", "IF", "<sup>Lignin&thinsp;</sup>&frasl;<sub>cellulose</sub>", "<i>ccr1-3</i> IF (traces of lignin)",
  "cad4x5", "IF", "<sup>G<sub>CHO&thinsp;</sub></sup>&frasl;<sub>G<sub>CHOH</sub></sub>", "<i>cad4 cad5</i> IF (overaccumulates G<sub>CHO</sub>)"
)

ref_data <- rmn_ref_avg %>%
  select(
    genotype,
    cell_type,
    "<sup>Lignin&thinsp;</sup>&frasl;<sub>cellulose</sub>" = lig.cellu,
  ...
  rename("actual_cell_type" = cell_type) %>% 
  mutate(cell_type = c("IF", "MX", "IF"),
         cell_type = ordered(cell_type,
      levels = c(
        "IF",
        "LP",
...
stat_tab <- violin_data_avg %>%
  ...
  anova_tab <- stat_tab$anova %>% 
    add_column(
      ...
      "cell_type" = stat_tab$cell_type) %>% 
    distinct()
  write_csv(anova_tab, paste0(stat_path, "05B_anova.csv"))
  ...
  tukey_tab <- stat_tab$tukey %>%
    add_column(
      ...
      "cell_type" = stat_tab$cell_type) %>% 
    distinct()
  write_csv(tukey_tab, paste0(stat_path, "05B_tukey.csv"))
```
...
stat_tab <- violin_data_avg %>%
  ...
  anova_tab <- stat_tab$anova %>% 
    add_column(
      "variable" = stat_tab$variable) %>% 
    distinct()
  write_csv(anova_tab, paste0(stat_path, "S06A_anova.csv"))

  tukey_tab <- stat_tab$tukey %>%
    add_column(
      "variable" = stat_tab$variable) %>% 
    distinct()
  write_csv(tukey_tab, paste0(stat_path, "S06A_tukey.csv"))
```
...
violin_data <- rmn_lac_sum %>%
  select(
    ...
    "G<sub>CHOH</sub>" = GOH_rel,
    "G<sub>CHO</sub>" = GCHO_rel,
    # "<sup>G<sub>CHO&thinsp;</sub></sup>&frasl;<sub>G<sub>CHOH</sub></sub>" = GCHO.GOH
  ) %>%
  filter(cell_type != "LP") %>%
  pivot_longer(
...
ref_labels <- tribble(
  ~genotype, ~cell_type, ~variable, ~label,
  "fah1", "MX", "S", "<i>fah1</i> MX (traces of S)",
  "fah1", "MX", "G", "<i>fah1</i> MX (overaccumulates G)",
  # "cad4x5", "IF", "G<sub>CHOH</sub>", "<i>cad4 cad5</i> IF (depleted of G<sub>CHOH</sub>)",
  "cad4x5", "IF", "G<sub>CHO</sub>", "<i>cad4 cad5</i> IF (overaccumulates G<sub>CHO</sub>)"
)
...
letters <- letter_groups(violin_data,
  ...
  mutate(value = case_when(
    cell_type == "MX" & variable != "Cellulose cryst." & variable != "P" ~ value - 0.3 * value,
    cell_type == "MX" & variable == "P" ~ value - 0.04,
...
lig_violin_WT + 
  wrap_elements(full = lig_violin + theme(plot.margin = unit(c(5, 2, 0, 2.4), "mm")), clip = TRUE) +
  # lig_violin +
...
# Mäule correlations and figure

## Mäule hue and S/G ratio

```{r}
maule_avg <- maule_hue %>% 
  group_by(genotype, cell_type) %>% 
  summarise(hue = mean(hue, na.rm = TRUE))

rmn_maule <- rmn_lac_sum %>% 
  group_by(genotype, cell_type) %>% 
  summarise("S/G" = mean(S.G, na.rm = TRUE),
            "S" = mean(S_rel, na.rm = TRUE)) %>% 
  mutate(genotype = ordered(case_when(
      genotype == "WT" ~ genotype,
      TRUE ~ paste0("<i>", str_replace(genotype, "-", "−"), "</i>")
    ),
  ...
  left_join(maule_avg) %>% 
  pivot_longer(c(`S/G`, S), names_to = "Raman band", values_to = "value") %>% 
  mutate(value_sq = value^2) %>%
  filter(`Raman band` == "S/G") %>% 
  drop_na()

S.G_lm <- lm(hue ~ log(value), data = rmn_maule)
rsq <- summary(S.G_lm)$adj.r.squared
...
rmn_maule_plot <- ggplot(rmn_maule,
                         aes(x = log(value),
                             y = hue)) +
  geom_smooth(method = "lm",
              se = FALSE,
              ...
              linetype = 2) +
  geom_point(aes(colour = genotype)) +
  # geom_label(aes(colour = genotype,
  #               label = cell_type),
  #           label.r = unit(1.2, "mm"),
  ...
  #           size = ggtext_size) +
  annotate("richtext",
           label = paste0("R<sup>2</sup> = ", round(rsq, digits = 2)),
           y = 410,
           x = 1.7,
           family = "Helvetica",
           size = ggtext_size,
  ...
  geom_tile(
    ...
    aes(fill = colour,
        ...
        width = 0.05,
        y = hue),
    position = position_nudge(y = 0, x = 1.94)
  ) +
  geom_text(data = bar_data %>% filter(colour == 9),
            position = position_nudge(y = 5, x = 2),
            label = "Visual hue scale",
            angle = -90,
            hjust = 0,
            family = "Helvetica",
  ...
  scale_y_continuous(limits = c(324, 416), 
                     ...
                     breaks = c(340, 360, 380, 400),
                     labels = c("340", "360", "20", "40")) +
  scale_color_manual(values = pal_dawn, guide = guide_legend()) +
  scale_fill_manual(values = hue_palette, guide = "none") +
  labs(y = "Mäule hue",
       x = "Log<sub><i>e</i></sub> Raman <sup>S</sup>/<sub>G</sub>") +
  theme_leo() +
  theme(axis.title.x = element_markdown(),
        ...
        plot.margin = unit(c(1, 7, 1, 1), "mm")) +
  coord_cartesian(xlim = c(0, 1.8), clip = "off")

pdf("maule_SG.pdf", height = onecol * 0.5, width = onecol)
rmn_maule_plot
dev.off()
```

## Mäule intensity and total lignin
...
rmn_maule <- rmn_lac_sum %>% 
  group_by(genotype, cell_type) %>% 
  summarise("Total lignin" = mean(lig.cellu, na.rm = TRUE)) %>% 
  mutate(genotype = ordered(case_when(
      genotype == "WT" ~ genotype,
      TRUE ~ paste0("<i>", str_replace(genotype, "-", "−"), "</i>")
    ),
...
lig_lm <- lm(OD ~ `Total lignin`, data = rmn_maule)
rsq <- summary(lig_lm)$adj.r.squared
...
rmn_maule_OD <- ggplot(
  ...
  labs(
    y = "Mäule absorbance",
    x = "Raman <sup>lignin</sup>/<sub>cellulose</sub>"
  ) +
  theme_leo() +
  theme(
...
pdf("maule_lignin.pdf", height = onecol * 0.5, width = onecol)
rmn_maule_OD
dev.off()
```
...
pdf("maule.pdf", height = onecol * 1.5, width = twocol)
maule_hue_plot + maule_OD_plot + rmn_maule_plot + rmn_maule_OD +
  plot_layout(design = layout) &
  plot_annotation(
    tag_levels = "A"
  ) &
...
stat_tab <- CC_data_avg %>% 
  ...
  anova_tab <- stat_tab$anova %>% 
    ...
    distinct()
  write_csv(anova_tab, paste0(stat_path, "06D_anova.csv"))

  tukey_tab <- stat_tab$tukey %>%
    add_column(
      "variable" = "CC autofluorescence realtive to CML") %>% 
    distinct()
  write_csv(tukey_tab, paste0(stat_path, "06D_tukey.csv"))
```
...
stat_tab <- CC_abs_avg %>%
  ...
  anova_tab <- stat_tab$anova %>% 
    ...
    distinct()
  write_csv(anova_tab, paste0(stat_path, "S10B_anova.csv"))

  tukey_tab <- stat_tab$tukey %>%
    add_column(
      "layer" = stat_tab$layer) %>% 
    distinct()
  write_csv(tukey_tab, paste0(stat_path, "S10B_tukey.csv"))
```
...
mag_files <-
  list.files(
    path = c(paste0(datapath, "PhD/UV-autofluorescence/William/Qmutants/")),
    pattern = "*.csv",
    recursive = TRUE,
    full.names = TRUE
  )
...
read_profiles <- function(x) {
  read_csv(x, show_col_types = FALSE) %>%
    mutate(filename = basename(x),
           genotype = case_when(str_detect(filename, "q_") ~ "<i>Q</i>",
                                TRUE ~ WT),
           genotype = ordered(genotype, levels = c("WT", "<i>Q</i>")),
           magnification = case_when(str_detect(filename, "10x") ~ "10x",
                                     str_detect(filename, "20x") ~ "20x",
                                     TRUE ~ "40x")) %>% 
    select(-filename) %>% 
    pivot_longer(-c("genotype", "magnification"), names_to = "ROI", values_to = "value") %>%
    mutate(ROI = str_remove(ROI, "Roi_"))
}

mag_data <- map_dfr(mag_files, read_profiles) %>% 
  group_by(genotype, magnification, ROI) %>% 
  mutate(
    pixel = row_number(),
    CC = median(pixel),
    position = pixel - CC,
...
mag_diff <- mag_data %>% 
  filter(layer == "CC") %>% 
  group_by(genotype, magnification) %>% 
  summarise(mean_scaled_CML = mean(scaled_CML))

mag_diff$position <- c(2.5, 2.7, 2.5, 2.5, 2, 2)
```

#### Plot profiles

```{r}
CC_profiles <- ggplot(mag_data,
       aes(x = position,
           y = scaled_CML,
           colour = genotype)) +
  annotate("rect",
           ...
           xmax = c(-30, 4, 25),
           fill = "grey95") +
  geom_line(aes(group = interaction(genotype, ROI)),
                size = 0.2,
                alpha = 0.5) +
  geom_smooth(method = "loess",
              span = 0.2,
  ...
  facet_wrap(~magnification, ncol = 1)
# plotly::ggplotly(CC_profiles)
pdf("mag_plot.pdf", width = onecol, height = onecol)
CC_profiles
dev.off()
```

#### Magnification violins

```{r}
mag_plot <- ggplot(
  mag_data %>% filter(layer == "CC"),
  aes(
    x = genotype,
    y = scaled_CML
  )
  ...
  geom_text(
    data = mag_diff,
    aes(label = paste0(round(mean_scaled_CML * 100, digits = 0), "%"),
        y = position),
    size = ggtext_size,
    family = "Helvetica"
  ) +
  labs(y = "CC autofluorescence normalised to CML") +
  scale_fill_manual(values = pal_dawn[c(1, 7)]) +
  # scale_y_continuous(breaks = c(0.5, 0.75, 1)) +
  # scale_shape_manual(values = c(21, 22, 23, 24, 25)) +
  theme_leo() +
  theme(
...
pdf("mag_violins.pdf", width = onecol, height = onecol * 0.5)
mag_plot
dev.off()
```
...
```{r}
pdf("mag_validation.pdf", width = onecol, height = onecol)
CC_profiles + mag_plot +
...
collapse_avg_sx <- collapse_stats %>% 
  group_by(genotype, cell_type, replicate) %>% 
  summarise("Solidity" = mean(Solidity))
...
letters <- letter_groups(collapse_avg_sx,
  Solidity,
  genotype,
  "tukey",
  cell_type,
  ...
  select(-Solidity) %>% 
  left_join(letters_tech, by = c("genotype", "cell_type"))
...
collapse_plot <- ggplot(
  ...
  labs(y = "TE convexity") +
  scale_color_manual(values = pal_dawn, aesthetics = c("colour", "fill")) +
  scale_y_continuous(limits = c(0.7, 1), breaks = c(0.5, 0.75, 1)) +
  # scale_x_discrete(limits = c(0, 1)) +
  # scale_shape_manual(values = c(21, 22, 23, 24, 25)) +
  theme_leo() +
  theme(
...
stat_tab <- collapse_avg %>%
  ...
  anova_tab <- stat_tab$anova %>% 
    ...
    distinct()
  write_csv(anova_tab, paste0(stat_path, "S11B_anova.csv"))
  ...
  tukey_tab <- stat_tab$tukey %>%
    ...
    distinct()
  write_csv(tukey_tab, paste0(stat_path, "S11B_tukey.csv"))
```
...
stat_tab <- perim_length_avg %>%
  ...
  anova_tab <- stat_tab$anova %>% 
    ...
    distinct()
  write_csv(anova_tab, paste0(stat_path, "S12A_anova.csv"))
  ...
  tukey_tab <- stat_tab$tukey %>%
    ...
    distinct()
  write_csv(tukey_tab, paste0(stat_path, "S12A_tukey.csv"))
```
...
### Correlations with Raman

#### Format data

```{r}
rmn_corr <- rmn_lac_sum %>% 
  select(
    lig.cellu,
    S.G,
    GCHO.GOH,
  ...
  group_by(genotype, replicate, cell_type) %>% 
  summarise(across(where(is.numeric), ~ mean(.x, na.rm = TRUE))) %>% 
  left_join(collapse_avg %>% mutate(cell_type = str_remove(cell_type, " convexity"))) %>% 
  select(-geometry) %>% 
  drop_na()
```
...
PX_data <- rmn_corr %>% 
  filter(cell_type == "PX") %>% 
  ungroup() %>% 
  select(-c(genotype, replicate, cell_type))

# Fit the full model 
full_model <- lm(Solidity ~., data = PX_data)
...
summary(lm(Solidity ~ S.G, data = PX_data))
```
...
# Fit the full model 
full_model <- lm(Solidity ~., data = MX_data)
# Stepwise regression model
step_model <- MASS::stepAIC(full_model, direction = "both", 
                      trace = FALSE)
summary(step_model)
...
summary(lm(Solidity ~ lig.cellu, data = MX_data))
```
...
stat_tab <- swelling_data_avg %>%
  ...
  anova_tab <- stat_tab$anova %>% 
    ...
    distinct()
  write_csv(anova_tab, paste0(stat_path, "08B_anova.csv"))
  ...
  tukey_tab <- stat_tab$tukey %>%
    ...
    distinct()
  write_csv(tukey_tab, paste0(stat_path, "08B_tukey.csv"))
```
...
absolute_thick <- ggplot(
  ...
  facet_wrap(~cell_type, strip.position = "left")

pdf("swelling_supp.pdf", width = onecol, height = onecol * 0.9)
perimeter_length + absolute_thick +
...
stat_tab <- swelling_data_avg %>%
  ...
  anova_tab <- stat_tab$anova %>% 
    ...
    distinct()
  write_csv(anova_tab, paste0(stat_path, "S12B_anova.csv"))
  ...
  tukey_tab <- stat_tab$tukey %>%
    ...
    distinct()
  write_csv(tukey_tab, paste0(stat_path, "S12B_tukey.csv"))
```
...
```{r}
options(knitr.kable.NA = '')
geno_data <- read_csv("/home/leonard/Dropbox/2021_lac_manuscript/Expression/primers.csv")
...
readr::write_file(geno_tab, "lac_table_qPCR_primers.txt")
```

## PCA

```{r}
# The original code for the root angle analysis got lost during a backup mishap.
...
pca_data <- rmn_lac_sum %>%
  filter(cell_type %in% c("IF", "XF", "PX", "MX")) %>%
  select(
    genotype,
    cell_type,
    replicate,
  ...
  unite("ID", genotype, cell_type, replicate, cell)
```
...
gg_pca <- as_tibble(pca_result$x, rownames = NA) %>%
    ...
    separate(ID, sep = "_", into = c("genotype", "cell_type", "replicate", "cell"))

gg_pca_centroid <- gg_pca %>%
    group_by(genotype) %>%
    summarise(
      PC1 = mean(PC1),
      PC2 = mean(PC2)
...
```{r}
pyro_data <- read_csv("/home/leonard/Dropbox/2021_lac_manuscript/Biochemistry/pyrolysis.csv") %>%
  filter(type == "hypocotyl") %>%
  mutate(
    lignin_raw = H + C + G + `5H` + S + P,
    "Total lignin [% of pyrolysate]" = lignin_raw / SUM * 100,
    "G [% of lignin]" = G / lignin_raw * 100,
    "S [% of lignin]" = S / lignin_raw * 100,
    "H [% of lignin]" = H / lignin_raw * 100,
    "5H [% of lignin]" = `5H` / lignin_raw * 100,
...
##### Plot mutant overview

```{r}
letters <- letter_groups(pyro_data,
                         value,
                         sample,
                         "tukey",
                         variable,
...
pyro_plot <- ggplot(pyro_data,
                    aes(x = sample,
                        y = value)) +
  geom_col(data = pyro_avg,
           aes(y = mean,
               fill = sample)) +
  geom_quasirandom(
    shape = 16,
  ...
  scale_fill_manual(values = c(pal_dawn[1], "grey80", "grey80", "grey80", pal_dawn[3])) +
  scale_x_discrete(labels = c("WT", "<i>ccr1-3</i>", "<i>fah-1</i>", "<i>cad4<br>cad5</i>", "<i>Q−5</i>")) +
  scale_y_continuous(labels = clean_zero) +
  theme_leo() +
  theme(strip.text.y.left = element_textbox_simple(halign = 0.5, 
                                               orientation = "left-rotated"),
        strip.placement = "outside",
        axis.text.x = element_markdown(vjust = 1),
        axis.title = element_blank()) +
  facet_wrap(~ variable,
             scales = "free_y",
             strip.position = "left")

pdf("biochem_supp.pdf", width = twocol, height = onecol)
pyro_plot
dev.off()
```
...
stat_tab <- pyro_data %>%
  drop_na(value) %>% 
  rowwise() %>%
  mutate(genotype = strip_html(sample),
         variable = str_remove_all(strip_html(variable), fixed("*"))) %>%
  ...
  anova_tab <- stat_tab$anova %>% 
    add_column(
      "variable" = stat_tab$variable) %>% 
    distinct()
  write_csv(anova_tab, paste0(stat_path, "S07_anova.csv"))

  tukey_tab <- stat_tab$tukey %>%
    add_column(
      "variable" = stat_tab$variable) %>% 
    distinct()
  write_csv(tukey_tab, paste0(stat_path, "S07_tukey.csv"))
```

### [unused] Thioacidolysis-FID

#### Load data

```{r, eval=FALSE}
pyro_lignin <- pyro_data %>% 
  filter(variable == "Total lignin") %>% 
  group_by(sample) %>% 
  summarise(total_lignin = mean(value, na.rm = TRUE))

thio_data <- read_csv("/home/leonard/Dropbox/2021_lac_manuscript/Biochemistry/thio.csv") %>%
  group_by(type, sample, replicate, method) %>% 
  mutate(
    # across(where(is.numeric), ~ .x / 100),
    "<i>O</i>–4-linked <sup>S</sup>&frasl;<sub>G</sub>" = sum(SCHO_term, SCHO_int, SCHOH_int, na.rm = TRUE) / sum(GCHO_term, GCHO_int, GCHOH_int, na.rm = TRUE),
    "<i>O</i>–4-linked <sup>S<sub>CHO&thinsp;</sub></sup>&frasl;<sub>S<sub>CHOH</sub></sub>" = sum(SCHO_term, SCHO_int, na.rm = TRUE) / (SCHOH_int),
    "<i>O</i>–4-linked <sup>G<sub>CHO&thinsp;</sub></sup>&frasl;<sub>G<sub>CHOH</sub></sub>" = sum(GCHO_term, GCHO_int, na.rm = TRUE) / (GCHOH_int),
    sample = ordered(sample,
      levels = c(
        "WT",
        "<i>ccr1-3</i>",
  ...
  left_join(pyro_lignin) %>% 
  mutate(
    "<i>O</i>–4-linked G" = sum(GCHO_term, GCHO_int, GCHOH_int, na.rm = TRUE) / total_lignin,
    "<i>O</i>–4-linked S" = sum(SCHO_term, SCHO_int, SCHOH_int, na.rm = TRUE) / total_lignin,
    "<i>O</i>–4-linked G<sub>CHO</sub>" = sum(GCHO_term, GCHO_int, na.rm = TRUE) / total_lignin,
    "<i>O</i>–4-linked G<sub>CHOH</sub>" = sum(GCHOH_int, na.rm = TRUE) / total_lignin,
    "<i>O</i>–4-linked S<sub>CHO</sub>" = sum(SCHO_term, SCHO_int, na.rm = TRUE) / total_lignin,
    "<i>O</i>–4-linked S<sub>CHOH</sub>" = sum(SCHOH_int, na.rm = TRUE) / total_lignin,
    ) %>% 
  select(-c(SCHO_term, SCHO_int, SCHOH_int, GCHO_term, GCHO_int, GCHOH_int, total_lignin)) %>%
  pivot_longer(!c(type, sample, replicate, method), names_to = "variable", values_to = "value") %>% 
  mutate(variable = ordered(variable, levels = c(
    "<i>O</i>–4-linked G",
    "<i>O</i>–4-linked S",
    "<i>O</i>–4-linked G<sub>CHO</sub>",
    "<i>O</i>–4-linked G<sub>CHOH</sub>",
    "<i>O</i>–4-linked S<sub>CHO</sub>",
    "<i>O</i>–4-linked S<sub>CHOH</sub>",
    "<i>O</i>–4-linked <sup>S</sup>&frasl;<sub>G</sub>",
    "<i>O</i>–4-linked <sup>G<sub>CHO&thinsp;</sub></sup>&frasl;<sub>G<sub>CHOH</sub></sub>",
    "<i>O</i>–4-linked <sup>S<sub>CHO&thinsp;</sub></sup>&frasl;<sub>S<sub>CHOH</sub></sub>"
  )))
...
```{r, eval=FALSE}
letters <- letter_groups(thio_data,
                         value,
                         sample,
                         "tukey",
                         variable,
...
thio_plot <- ggplot(thio_data,
                    aes(x = sample,
                        y = value)) +
  geom_col(data = thio_avg,
           aes(y = mean,
               fill = sample)) +
  geom_quasirandom(
    shape = 16,
...
```{r, eval=FALSE}
pdf("biochem_supp.pdf", height = twocol, width = twocol)
pyro_plot + thio_plot +
...
expression_edouard <- read_csv("/home/leonard/Dropbox/2021_lac_manuscript/Expression/expression_edouard.csv") %>% 
  select(-ref) %>% 
...
results <- results %>%
  ...
  drop_na(sample) %>%
  mutate(
    gene = ordered(gene, levels = c("LAC4", "LAC5", "LAC10", "LAC10 up", "LAC11", "LAC12", "LAC12 up", "LAC17", "UBI", "18S", "EF1a")),
...
#### Calculate dCt, ddCt and fold change
...
```{r}
bar_data <- normalised %>% filter(metric == "fold" & gene != "<i>EF1a</i>" & sample != "WT")
bar_data_avg <- normalised_avg %>% filter(metric == "fold" & gene != "<i>EF1a</i>" & sample != "WT")

letters <- letter_groups(bar_data,
                         value,
                         sample,
                         "tukey",
                         gene,
...
ex_bar <- ggplot(
  ...
  geom_text(data = letters,
            ...
            family = "Helvetica",
            size = ggtext_size) +
  scale_y_continuous(expand = expansion(mult = c(0.15, 0.05)), labels = clean_zero) +
  scale_fill_manual(values = pal_dawn[c(1:6)]) +
  labs(y = "Relative gene expression [% of WT]") +
  theme_leo() +
...
pdf("expression_bars.pdf", height = onecol * 0.6, width = twocol)
ex_bar
dev.off()
```
...
stat_tab <- bar_data %>%
  ...
  rowwise() %>%
  mutate(genotype = strip_html(sample),
         variable = str_remove_all(strip_html(gene), fixed("*"))) %>%
  group_by(genotype, variable) %>%
  mutate(
    "n" = n(),
    genotype = paste0(genotype, " (n = ", n, ")")
  ...
  anova_tab <- stat_tab$anova %>% 
    add_column(
      "variable" = stat_tab$variable) %>% 
    distinct()
  write_csv(anova_tab, paste0(stat_path, "S01B_anova.csv"))

  tukey_tab <- stat_tab$tukey %>%
    add_column(
      "variable" = stat_tab$variable) %>% 
    distinct()
  write_csv(tukey_tab, paste0(stat_path, "S01B_tukey.csv"))
```
...
ex_heatmap <- ggplot() +
  ...
  labs(
    # title = "Fold-change of laccase gene expression in single mutants",
    y = "Quantified amplicon",
    x = "Plant genotype"
    ) +
  theme(
    axis.title = element_blank(),
...
pdf("mutant_expression.pdf", height = onecol, width = onecol)
ex_heatmap
dev.off()
```
...
```{r}
scheme_genes <- read_csv("/home/leonard/Dropbox/2021_lac_manuscript/Expression/scheme_features.csv") %>% 
  group_by(gene) %>% 
  mutate(beginning = min(c(start, stop), na.rm = TRUE),
         beginning_real = beginning,
         end = max(c(start, stop), na.rm = TRUE),
         across(where(is.numeric), ~ .x - beginning),
         start_real = start,
         stop_real = stop,
         start = case_when(gene %in% c("LAC5", "LAC11") ~ (-stop_real) + end,
                           TRUE ~ start),
         stop = case_when(gene %in% c("LAC5", "LAC11") ~ (-start_real) + end,
                           TRUE ~ stop),
         forward = case_when(forward == FALSE ~ TRUE,
                           TRUE ~ forward),
         gene = ordered(gene, levels = c("LAC4", "LAC5", "LAC10", "LAC11", "LAC12", "LAC17"))
  )

scheme_labels <- read_csv("/home/leonard/Dropbox/2021_lac_manuscript/Expression/scheme_labels.csv") %>% 
  left_join(read_csv("/home/leonard/Dropbox/2021_lac_manuscript/Expression/scheme_features.csv") %>%
              group_by(gene) %>% 
              summarise(beginning = min(c(start, stop), na.rm = TRUE),
                        end = max(c(start, stop), na.rm = TRUE)) %>% 
              select(gene, beginning, end)) %>% 
  mutate(position = case_when(gene %in% c("LAC5", "LAC11") ~ (-position) + end, 
                              TRUE ~ position - beginning),
         gene = ordered(gene, levels = c("LAC4", "LAC5", "LAC10", "LAC11", "LAC12", "LAC17"))
  )
...
gene_scheme <- ggplot(
  ...
  ## forward
  geom_feature(
    data = scheme_labels %>% filter(type == "T-DNA" & forward == TRUE),
    aes(x = position, y = gene, forward = forward),
    feature_height = unit(4, "mm"),
    arrowhead_width = unit(2, "mm"),
    # feature_width = unit(5, "mm"),
    size = 0.8,
    colour = pal_ostwald_disc[3]
  ) +
  geom_feature_label(
    data = scheme_labels %>% filter(type == "T-DNA" & forward == TRUE),
    aes(x = position, y = gene, label = name, forward = forward),
  ...
  ## reverse
  geom_feature(
    data = scheme_labels %>% filter(type == "T-DNA" & forward == FALSE),
    aes(x = position, y = gene, forward = forward),
    feature_height = unit(-4, "mm"),
    arrowhead_width = unit(2, "mm"),
    size = 0.8,
  ...
  ## LP
  geom_feature(
    data = scheme_labels %>% filter(type == "Genotyping primer" & name == "LP"),
    aes(x = position, y = gene, forward = forward),
  ...
  ## RP
  geom_feature(
    data = scheme_labels %>% filter(type == "Genotyping primer" & name == "RP"),
    aes(x = position, y = gene, forward = forward),
    feature_height = unit(-3, "mm"),
    arrowhead_width = unit(1, "mm"),
    size = 0.8,
  ...
  ## FW
  geom_feature(
    data = scheme_labels %>% filter(type == "RT-qPCR primer" & name %in% c("FW", "FWup")),
    aes(x = position, y = gene, forward = forward),
    arrowhead_width = unit(1, "mm"),
    size = 0.8,
    colour = "black"
  ...
  ## RV
  geom_feature(
    data = scheme_labels %>% filter(type == "RT-qPCR primer" & name %in% c("RV", "RVup")),
    aes(x = position, y = gene, forward = forward),
    feature_height = unit(-3, "mm"),
    arrowhead_width = unit(1, "mm"),
    size = 0.8,
  ...
  # Cu binding sites
  geom_feature(
    data = scheme_labels %>% filter(type == "Cu"),
    aes(x = position, y = gene, forward = forward),
    feature_height = unit(-2, "mm"),
    size = 0.8,
    colour = "#B77729"
  ) +
  geom_segment(
  ...
  # rest of gene
  geom_gene_arrow(
    data = scheme_genes %>% filter(is.na(forward)),
  ...
  # geom_gene_label(aes(label = feature),
  #   align = "left",
  #   min.size = 6,
  #   size = 6,
  #   grow = FALSE,
  #   family = "Helvetica"
  # ) +
  scale_fill_manual(values = c(lighten(pal_ostwald_disc[1], 0.2), pal_ostwald_disc[2], "grey80")) +
  scale_x_continuous(expand = expansion(mult = c(0.02, 0.001)),
                     breaks = c(0, 1000, 2000, 3000),
                     labels = c("0 [5']", "1000", "2000", "3000 bp")) +
  theme_leo() +
  theme(
...
pdf("gene_models.pdf", width = twocol, height = onecol)
gene_scheme
dev.off()
```
...
pdf("Blaschek2022_figureS1_heatmap.pdf", height = twocol, width = twocol)
gene_scheme + ex_heatmap + plot_layout(
    design = layout
    ) &
  plot_annotation(
    tag_levels = "A"
...
pdf("Blaschek2022_figureS1_bars.pdf", height = twocol * 1.1, width = twocol)
gene_scheme + ex_bar + plot_layout(
    design = layout
    ) &
  plot_annotation(
    tag_levels = "A"
...
writexl::write_xlsx(list(
  "cell culture differentiation" = growth_data %>% 
    select(replicate, "days past induction" = dpi, treatment, "TE proportion" = TE_prop, "TE viability" = TE_viab, n, comment) %>% 
    arrange(`days past induction`, treatment, replicate),
  "cell culture gene expression" = expression,
  "mutant expression" = normalised %>% 
    rowwise() %>% 
    mutate(gene = strip_html(gene),
         sample = strip_html(sample)) %>% 
    select(-c(date, concentration, ref)),
  "pyrolysis-GCMS in cell cultures" = pyro_TE_data %>% 
    select(-method) %>% 
    rowwise() %>%
    mutate(variable = str_remove_all(strip_html(variable), fixed("*"))) %>% 
  ...
  "root length growth rate" = nested_root_data %>%
    ...
    rowwise() %>%
    mutate(genotype = strip_html(genotype)),
  "stem thickness and count" = thickness_data %>% 
    select(genotype, 'replicate' = rep, "stem height [cm]" = stretched.height, "stem diameter [cm]" = thickness, "diameter/height" = T.H, "stem count" = stems) %>% 
    rowwise() %>%
    mutate(genotype = strip_html(genotype)),
  "stem base angles" = angles %>% 
    select(genotype, replicate, dpg, "stem base angle [∆90°]" = base_d90) %>% 
    rowwise() %>%
    mutate(genotype = strip_html(genotype)),
  ...
  "Mäule hue" = maule_hue %>%
    ...
    rowwise() %>%
    mutate(genotype = strip_html(genotype)),
  "Mäule absorbance" = maule_OD %>% 
    rename("absorbance" = OD,
           "absorbance of the phloem" = PH_OD) %>% 
    rowwise() %>%
    mutate(genotype = strip_html(genotype)),
  ...
  "pyrolysis-GCMS in mutants" = pyro_data %>% 
    select(-method) %>% 
    rowwise() %>%
    mutate(variable = str_remove_all(strip_html(variable), fixed("*")),
           sample = strip_html(sample)) %>% 
    pivot_wider(names_from = variable, values_from = value),
  "UV-fluorescence by objective" = mag_data %>% 
    select(genotype, magnification, "line profile #" = ROI, "pixel value" = value, "position relative to CC" = position, "pixel value scaled to CML" = scaled_CML) %>% 
    rowwise() %>%
    mutate(genotype = strip_html(genotype)),
...
list_of_tables %>% 
  map(~pull(.,`Figure panel`)) %>% # Pull out panel variable
