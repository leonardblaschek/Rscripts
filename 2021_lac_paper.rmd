...
rmn_maule_plot <- ggplot(rmn_maule,
  ...
  facet_wrap(~ `Raman band`,
             ncol = 2,
             scales = "free_x")

rmn_maule_plot

rmn_maule_lm <- rmn_maule %>% 
  group_by(`Raman band`) %>% 
  group_split() 

S.G_lm <- lm(log(hue) ~ log(value), data = rmn_maule_lm[[2]])
summary(S.G_lm)

S_lm <- lm(log(hue) ~ log(value), data = rmn_maule_lm[[1]])
summary(S_lm)
```
