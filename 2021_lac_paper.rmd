...
ggplot(profile_data,
       aes(x = pixel,
           y = absorbance,
           colour = filename)) +
  geom_line(aes(group = interaction(filename, profile)),
            alpha = 0.25) +
  geom_smooth(se = F)
```
...
plotly::ggplotly(ggplot(test_data %>% filter(Well == "H6"),
       aes(x = wavelength,
           y = absorbance,
           group = time)) +
  geom_line(size = 0.2,
...
abs_max <- tibble(
  substrate = c("ABTS", "DAF", "MBTH", "PYGL", "SGZ"),
  abs_max = c(420, 690, 505, 450, 530), #DAF default: 610
  abs_min = c(480, 570, 700, 600, 600) #DAF default: 515
)
