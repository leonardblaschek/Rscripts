...
datapath <- ifelse(dir.exists("/data/"), "/data/", "/home/leonard/Documents/Uni/")
```
...
pdf("DAF_pH.pdf", width = twocol, height = onecol * 1.75)
activity_grid /
slopes /
ph_cont
dev.off()

# svglite::svglite("DAF_pH.svg", width = twocol, height = onecol * 1.75)
# activity_grid /
# slopes /
# ph_cont
# dev.off()
...
growth_long <- growth_data %>%
  ...
  pivot_longer(cols = c(TE_viab, TE_prop, nTE_viab),
               names_to = "variable",
               values_to = "value")
...
pdf("TE_growth_sigmoid.pdf", width = onecol, height = onecol * 0.5)
# growth
growth_facet
dev.off()
...
TE_plates <- map(plate_files, read_plus) %>%
  bind_rows()
```
...
#       date = ymd_hms(paste(date, time, sep = "_")),
#       started = case_when(hour(date) < 14 ~ "morning", 
#                           TRUE ~ "evening")
#     ) %>% 
#     select(-time)
# }
...
# bench::mark(
#   TE_spec <- map(spec_files, read_plus) %>% bind_rows(),
#   TE_spec <- map(spec_files, read_vroom) %>% bind_rows()
# )

TE_spec <- map(spec_files, read_vroom) %>%
  bind_rows() %>% 
  pivot_longer(c(`220`:`1000`), names_to = "wavelength", values_to = "measurement") %>%
...
## Plot end-point spectra

```{r}
plotly::ggplotly(
  ggplot(TE_data %>% filter(time == 390),
       aes(x = wavelength,
           y = zeroed_absorbance,
           linetype = sample,
           colour = pH)) +
...
abs_max <- tibble(
  substrate = c("ABTS", "DAF", "MBTH", "PYGL", "SGZ"),
  abs_max = c(420, 710, 505, 450, 525),
  # abs_min = c(480, 800, 700, 600, 600)
  abs_min = 1000
)
...
kinetics <- TE_data %>%
  ...
  left_join(lm_bounds) %>% 
  group_by(
    sample, substrate, pH, replicate, day, time, lm_start, lm_stop, density
    ) %>%
  mutate(corrected_absorbance = zeroed_absorbance - zeroed_absorbance[wavelength == abs_min]) %>% 
  summarise(
    corrected_absorbance = corrected_absorbance[wavelength == abs_max],
...
ggplot(kinetics %>% filter(sample != "neg. ctrl."),
       aes(x = time,
           y = corrected_absorbance,
           colour = pH,
           linetype = sample)) +
  # geom_line(aes(group = interaction(sample, pH))) +
  geom_smooth(aes(group = interaction(sample, pH)),
              size = 0.2) +
  # scale_x_continuous(limits = c(0, 400)) +
  theme_leo() +
  theme(legend.position = "bottom") +
...
ggplot(kinetics_nested %>% filter(sample != "neg. ctrl."),
       aes(x = day,
           y = scaled_estimate,
           colour = sample)) +
  geom_line(size = 0.2) +
