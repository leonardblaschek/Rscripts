...
EEFFGGG
"
pdf("Blaschek2022_figureS1.pdf", height = onecol, width = twocol)
roots + bolt_geno + bolt_inst + silique_plot + silique_linreg + hue_plot + hue_violin +
...
absolute_thick <- ggplot(swelling_data_fresh,
  ...
  annotate("segment",
           ...
           yend = c(0.8, 0.3, 0.8),
           size = 0.2) +
  labs(y = "MX Cell wall thickness [µm]") +
  scale_color_manual(values = rev(pal_dawn), aesthetics = c("colour", "fill")) +
  scale_y_continuous(limits = c(0, 5)) +
  theme_leo() +
  theme(axis.text.x = element_markdown())

pdf("fresh_MX.pdf", width = onecol, height = onecol * 0.35)
absolute_thick
dev.off()
```
...
pdf("DAF_pH.pdf", width = onecol * 0.5, height = onecol)
ph_cont
dev.off()
```
...
data_dab <- data_dab %>%
  ...
  mutate(genotype = ordered(case_when(
    ...
    cell_type = recode(cell_type, "CML" = "IF-CML"),
...
data_dab <- data_dab %>%
  filter(
    cell_type %in% c("IF", "IF-CML", "XF", "MX", "PX") &
      pH == 5 &
      treatment == "none"
    # adj.r.squared > 0.85
  )
...
ACCCCCC
"
pdf("section_activity.pdf", width = twocol, height = onecol * 0.9)
...
## [FAILS] Draw substrates

```{r}
# library("rcdk")
# daf_sdf <- load.molecules("https://pubchem.ncbi.nlm.nih.gov/rest/pug/compound/CID/10679/record/SDF/?record_type=2d&response_type=save&response_basename=Structure2D_CID_10679")
# depictor <- get.depictor(width=200, height=200)
# view.molecule.2d(daf_sdf[[1]])
```
...
pdf("Raman_supp_fig.pdf", width = twocol, height = onecol)
lig_violin 
dev.off()
```
