...
daf_IF_files <-
  list.files(
    path = paste0(
      datapath, "PhD/Laccase_activity/2020-09_activity/DAF_pH5_IF/ROIs/Measurements/"
    ),
    pattern = "*.csv$",
    recursive = FALSE,
    full.names = TRUE
...
reps <- read_csv(paste0(
  datapath, "PhD/Laccase_activity/2020-09_activity/DAF_pH5_IF/to_measure_IF.csv")
) %>% 
  select(1:3) %>% 
...
daf_IF_data <- map_dfr(daf_IF_files, read_csv) %>%
  ...
  mutate(
    ...
    genotype = str_extract(image, "WT|Q-4|Q-5|Q-10|Q-12|Q-17|Q$"),
    treatment = "none",
    pH = "5",
    substrate = "DAF"
  ) %>%
