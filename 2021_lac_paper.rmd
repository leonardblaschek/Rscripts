...
pdf("lac_Wiesner.pdf", width = twocol, height = onecol)
Wiesner_plot
dev.off()
```
...
pdf("DAF_pH.pdf", width = twocol, height = onecol * 1.5)
activity_grid /
slopes /
ph_cont
dev.off()
...
kinetics <- tbl(TE_db, "TE_db") %>%
  left_join(abs_max, copy = TRUE) %>%
  left_join(lm_bounds, copy = TRUE) %>% 
  group_by(
    date, sample, substrate, pH, replicate, day, time, lm_start, lm_stop, density
    ) %>%
  summarise(
    # corrected_absorbance = zeroed_absorbance[wavelength == abs_max] - 
    #   zeroed_absorbance[wavelength == abs_min],
...
```{r}
plotly::ggplotly(
  ggplot(TE_data %>% filter(time == 380 & sample == "TE"),
       aes(x = wavelength,
           y = zeroed_absorbance,
           linetype = sample,
           colour = pH)) +
  geom_line(aes(group = interaction(replicate, pH)),
            size = 0.2) +
  # scale_x_continuous(limits = c(0, 400)) +
  theme_leo() +
  theme(legend.position = "bottom") +
...
CCCCC#
"
pdf("TE_activity_slopes_TEspec.pdf", width = onecol * 1.5, height = onecol)
...
lig_violin("G-CHOH", "IF") +
lig_violin("Total lignin", "XF") +
lig_violin("G-CHOH", "XF") +
lig_violin("Total lignin", "MX") +
lig_violin("G-CHOH", "MX") +
  plot_layout(ncol = 2)
dev.off()
```
