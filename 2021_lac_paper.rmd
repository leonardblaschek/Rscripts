...
nested_stem_data <- stem_data %>%
  filter(dpf >= lm_start & dpf <= lm_stop & genotype %in% c(WT, quad_it, Q_it)) %>%
  select(genotype, replicate, height, dpf) %>%
  nest(data = c(dpf, height)) %>%
  mutate(
    fit = map(data, ~ lm(height ~ dpf, data = .x)),
    tidied = map(fit, tidy),
    model = map(fit, glance),
    adj.r.squared = unlist(map(model, ~ .x[["adj.r.squared"]]))
  ) %>%
