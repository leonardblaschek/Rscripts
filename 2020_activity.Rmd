...
#### subtract water column and background ####
data <- data %>%
  filter(substrate == "DAB" & date != "2020-07-07") %>%
  # anti_join(read_csv(
...
nested_data <- data %>%
  ...
  filter(PH_slope < 0.00025 & date != "2020-07-07")
# %>%
...
unfiltered_data <- data %>% 
  filter(cell_type %in% c("IF", "XF", "MX", "PX", "LP", "PH") &
  pH == 5 &
  treatment == "none")
...
pdf("DAB_pH5.pdf", width = twocol * 1.5, height = onecol * 1.5)
raw_absorbance / 
  activity_grid /
  slopes
dev.off()
