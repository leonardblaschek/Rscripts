...
hyp_dist <- ggplot(hyp_data, aes(x = Solidity)) +
  ...
  scale_colour_manual(values = c("black", "black"), aesthetics = "fill") +
  # scale_y_discrete(labels = c("Water", "Drought")) +
...
absolute_thick <- ggplot(swelling_data_fresh, aes(x = thickness)) +
  geom_density_ridges(data = swelling_data_fresh %>% filter(genotype == "WT"),
                      aes(y = 1, fill = genotype), 
                      colour = "white",
                      size = 0.2,
                      quantile_lines = TRUE,
                      quantiles = 2,
                      scale = 0.5) +
  geom_density_ridges(data = swelling_data_fresh %>% filter(genotype == "<i>Q</i>"),
                      aes(y = 1, fill = genotype), 
                      colour = "white",
                      size = 0.2,
                      quantile_lines = TRUE,
  ...
  geom_hline(yintercept = c(1),
              colour = "white",
             size = 0.2) +
  # geom_point(data = swelling_data_avg %>% filter(genotype == "WT"),
  #            aes(y = condition),
  #            fill = "#ffcc3d",
  #            colour = "white",
  #            position = position_nudge(x = 0, y = 1),
  ...
  #            size = 2,
  #            stroke = 0.2) +
  # geom_point(data = swelling_data_fresh %>% filter(genotype == "<i>Q</i>"),
  #            aes(y = condition),
  #            fill = "#ffcc3d",
  #            colour = "white",
  #            position = position_nudge(x = 0, y = -1),
  ...
  #            size = 2,
  #            stroke = 0.2) +
  labs(x = "Cell wall thickness [µm]",
       y = "") +
  # scale_colour_manual(values = pal_ostwald_disc[c(3,1)], aesthetics = "fill") +
  # scale_colour_manual(values = c("black", "black"), aesthetics = "fill") +
  scale_colour_manual(values = pal_dawn[c(7,1)], aesthetics = "fill") +
  # scale_y_discrete(labels = c("Water", "Drought")) +
  # scale_x_continuous(limits = c(0.75, 1), breaks = c(0.8, 0.9, 1)) +
  theme_leo() +
  theme(strip.text = element_markdown(),
