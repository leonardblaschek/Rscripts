...
# tukey <- function(x) {
#   aov1 <- aov(data = x, absorbance ~ interaction(genotype, position))
#   groups <- HSD.test(aov1, "interaction(genotype, position)", alpha = 0.001)
#   groups$groups[["genotype"]] <- rownames(groups$groups)
#   groups$groups[["absorbance"]] <- -0.05
#   return(groups[["groups"]])
# }
# 
# box_letters <- cml_data_box[, -c(1, 3, 4, 5)] %>%
#   # group_by(position) %>%
#   do(data.frame(tukey(.))) %>% 
#   separate(genotype, into = c("genotype", "position"), sep = "\\.")
...
pdf("cml_box_across.pdf", width = 14)
layer_boxes
dev.off()
