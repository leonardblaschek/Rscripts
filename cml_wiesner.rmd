...
cml_data <- read_csv(
  ...
  mutate(
    ...
    # distance.scaled = (distance - min(distance)) / (max(distance) - min(distance)),
    absorbance.scaled = scale(absorbance, center = FALSE, scale = TRUE),
    # absorbance.scaled = (absorbance - min(absorbance)) / (max(absorbance) - min(absorbance))
    )

head(cml_data)
```

```{r scaled line plots, fig.showtext = TRUE, out.width = '100%', message = FALSE}
...
```{r unscaled line plots, fig.showtext = TRUE, out.width = '100%', message = FALSE}
