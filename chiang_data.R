...
wang2018_export <- full_join(comp, sacc) %>%
  full_join(wood) %>%
  full_join(moe)

  write.csv(wang2018_export, "Wang2018_data.csv")
...
#   ))[["adj.r.squared"]], 2)
r2.moe <-
  round(summary(lm((Aldehydes / Lignin.) ~ MOE,
             data = filter(wang2018_export, Aldehydes > 0)
  ))[["adj.r.squared"]], 2)
# r2.sg <-
...
chiang.scatter <-
  ggplot(data = wang2018_export, aes(x = (Aldehydes / Lignin.) * 100, y = MOE)) +
  xlim(0.01, 2.5) +
  annotate(
    "text",
    family = "Helvetica",
