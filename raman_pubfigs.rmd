...
py_rmn <- rmn_data_avg %>%
  filter(wavenumber %in% c(1099, 1276, 1334, 1601, 1603, 1625, 1664)) %>%
  group_by(genotype, wavenumber) %>%
  mutate(mean.scaled = case_when(wavenumber == 1601 ~ mean.lig_peak,
                                 TRUE ~ mean.scaled)) %>%
...
# pdf("pyro_correlations.pdf", height = 20, width = 20)
ggplot(data = filter(py_rmn_corr, variable == "mean")) +
  geom_rect(
    data = py_rmn_corr_mat,
    xmin = -Inf,
    xmax = Inf,
