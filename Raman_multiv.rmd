...
```{r opls-da iterate, fig.show="hide", warning = FALSE, message = FALSE}
...
```{r S-peaks, fig.showtext = TRUE, out.width = '100%', warning = FALSE, message = FALSE}
library(tukeygrps)
rmn_data_wavenumbers <- rmn_data %>%
  select(-intensity, -corrected.intensity) %>%
  filter(cell.type %in% c("IF", "PMX") & genotype != "ccr1xfah1") %>%
...
# pdf("raman_peaks.pdf", width = 7, height = 7)
ggplot(data = rmn_data_wavenumbers, aes(x = genotype, y = scaled)) +
