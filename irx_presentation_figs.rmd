...
eds_data_ratio <- eds_data %>% 
  group_by(days) %>% 
  mutate(PCW_C = mean(C[layer == "PCW"]),
         PCW_O = mean(O[layer == "PCW"])) %>% 
  filter(layer == "SCW") %>% 
  mutate("C/O" = C / O,
         "PCW_C/O" = PCW_C / PCW_O,
         "SCW/PCW" = `C/O` / `PCW_C/O`) %>% 
  ungroup() %>% 
  add_row(tibble(layer = "SCW", days = "0", "SCW/PCW" = 1))

eds_data_avg <- eds_data_ratio %>% 
  group_by(days) %>% 
  summarise("EDS_mean" = mean(`SCW/PCW`),
            "EDS_mean" = mean(`SCW/PCW`))

ggplot(eds_data_ratio, aes(x = days, y = `SCW/PCW`)) +
  geom_quasirandom() +
...
ipsc <- eds_data_ratio %>% 
  select(days, `SCW/PCW`) %>% 
  pivot_longer(`SCW/PCW`, names_to = "variable", values_to = "value") %>% 
  add_row(em_data %>% filter(treatment == "AD" &
                                 state == "Intact") %>%
              ungroup() %>% 
              select(days, proportion) %>% 
              add_row(tibble(days = "0", proportion = 0)) %>% 
              pivot_longer(proportion, names_to = "variable", values_to = "value")) %>% 
  add_row(afm_data %>% filter(cell.type == "MX" & variable == "modulus") %>% 
              ungroup() %>% 
              select(days, ratio) %>% 
              add_row(tibble(days = "0", ratio = 1)) %>% 
              pivot_longer(ratio, names_to = "variable", values_to = "value")) %>%
  mutate(variable = ordered(recode(variable,
                                   "SCW/PCW" = "C/O ratio",
                                   "ratio" = "Stiffness",
                                   "proportion" = "Resistance [%]"),
                            levels = 
                              c("C/O ratio", "Stiffness", "Resistance [%]"))) %>%
  group_by(variable) %>% 
  mutate(scaled = value / max(value))

pdf("ipsc_fig.pdf", width = onecol, height = onecol)
ggplot(ipsc, aes(x = days, y = value)) +
  stat_summary(aes(group = 1), geom = "line", fun = "median", linetype = 2) +
  geom_quasirandom(aes(fill = scaled),
                   size = 2,
                   width = 0.1,
                   shape = 21,
                   stroke = 0.1) +
  scale_fill_gradientn(colours = morgenstemning) +
...
violin_strength <- flex_violin("max.stress") +
  ...
  scale_x_discrete(
    labels = c(
      "Wild type",
      expression(paste(Increased~G[CHOH]~italic("fah1"))),
      expression(paste(italic("cad4"), "x", italic("5")))
    ),
...
violin_stiffness <- flex_violin("modulus") +
  ...
  scale_x_discrete(
    labels = rev(c(
      "Wild type",
      "Increased <b>G</b><sub>CHOH</sub><br><i>(fah1)</i>",
      "Increased <b>G</b><sub>CHO</sub><br><i>(cad4</i>x<i>5)</i>"
    )),
    limits = rev(levels(stiffness_complete$genotype))
  ) +
  theme(axis.title.y = element_blank(),
...
pdf("bending_fig.pdf", width = onecol, height = onecol)
violin_stiffness |
violin_breakpoint
dev.off()
...
recovery_data <- read_csv(paste0(datapath, "PhD/IRX/Drought/recovery.csv")) %>%
  mutate(
    ...
    genotype = 
      ordered(recode(genotype, 
                     "WT" = "Wild type",
                     "cad4x5" = "Increased <b>G</b><sub>CHO</sub><i> (cad4</i>x<i>5)</i>"),
              levels = c(
                "Wild type", 
                "Increased <b>G</b><sub>CHO</sub><i> (cad4</i>x<i>5)</i>"))
  ) %>%
  group_by(genotype, medium) %>%
  mutate(sum = sum(count)) %>%
...
pdf("recovery.pdf", width = onecol * 1.5, height = onecol * 0.65)
recovery_collapse
dev.off()
```
