...
py_plot_lig <- py_plot("Lignin", "1603/4.6 + 1334") +
  labs(x = expression(paste("Total lignin * total pyrolysate"^-1)),
    y = expression(paste("Composite lignin band [AU * AUC"^-1,"]"))) +
  scale_y_continuous(breaks = c(0, 0.0015, 0.003, 0.0045))
...
py_plot_cho <- py_plot("GCHO/lig", "1625/lig", "cad4xcad5") +
  labs(x = expression(paste("Total G-aldehydes * total lignin"^-1)),
    y = expression(paste("1625 cm"^-1,"/ composite lignin band"))) +
...
rmn_data_corr <- rmn_data_corrected %>%
  filter(wavenumber %in% c(1664, 1625, 1334)) %>%
  # group_by(genotype, cell.type, replicate, technical) %>%
  mutate(wavenumber = as.character(wavenumber)) %>%
  ungroup() %>%
  pivot_wider(
