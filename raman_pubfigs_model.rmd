...
#### Export model spectra
```{r model compounds, fig.showtext = TRUE, out.width = '100%', warning = FALSE, message = FALSE, echo = FALSE}

rmn_model_export_raw <- rmn_models %>%
  select(state:intensity) %>%
  pivot_wider(id_cols = c(state:gamma), names_from = wavenumber, values_from = intensity) %>%
  select(order(as.numeric(as.character(colnames(.))))) %>%
  select(state:gamma, everything())

rmn_model_export_scaled <- rmn_models %>%
  select(state:wavenumber, scaled) %>%
  pivot_wider(id_cols = c(state:gamma), names_from = wavenumber, values_from = scaled) %>%
  select(order(as.numeric(as.character(colnames(.))))) %>%
  select(state:gamma, everything())
  

write_excel_csv(rmn_model_export_raw, "model_spectra_raw.csv")
write_excel_csv(rmn_model_export_scaled, "model_spectra_scaled.csv")
```


#### Plot figure 1

```{r model compounds plot, fig.showtext = TRUE, out.width = '100%', warning = FALSE, message = FALSE, echo = FALSE}
#### Upset plot ####
...
peak_upset <- ggplot(peak_pres) +
  ...
  scale_y_discrete(labels = rev(c(
    expression(paste(bold("G")[CHOH])),
    expression(paste(bold("G")[CHOH])),
    expression(paste(bold("G")[sat - CHOH])),
    expression(paste(bold("G")[CHO])),
    expression(paste(bold("G")[CHO])),
    expression(paste(bold("G")[COOH])),
    expression(paste(bold("S")[CHOH])),
    expression(paste(bold("S")[CHO])),
    expression(paste(bold("S")[CHO])),
    expression(paste(bold("S")[COOH])),
    expression(paste(bold("5H")[COOH])),
    expression(paste(bold("C")[COOH])),
    expression(paste(bold("H")[COOH]))
  ))) +
...
pdf("Raman_figure1.pdf", width = twocol, height = 6)
...
heatmap <- ggplot(loadings_long) +
  ...
  guides(fill = guide_colourbar(title = "Absolute\nloading")) +
  theme_leo() +
  theme(
...
loadings_avg <- ggplot(data = rmn_data_loading, aes(
  ...
  theme_leo() +
  guides(fill = guide_colorbar(title = "Mean absolute\nloading")) +
...
total_corr <- ggplot(data = py_rmn_corr) +
  ...
  theme(
    ...
    strip.text = element_text(size = 8),
    legend.key.width = unit(4, "mm"),
    plot.margin = unit(c(0.75, 2, 2, 2), "mm")
  ) +
...
pdf("Raman_figure5.pdf", width = twocol, height = 6)
...
schemes <-
  rasterGrob(image_read_pdf(
    "/home/leonard/Dropbox/Raman manuscript/Submission/ACS/2020-02_revision/Edits/Raman_figure7.pdf"
  ))

pdf("Raman_figure6.pdf", width = 5, height = 6)
plot_grid(
  rel_auc,
  rel_lig,
  schemes,
...
rmn_fluo_auc <- rmn_models_fluo %>%
  ...
  left_join(read_csv("/home/leonard/Documents/Uni/PhD/Raman manuscript/R data/monomer_params.csv")) %>%
  filter(wavenumber %in% c(300:1700)) %>%
  group_by(filename) %>%
...
supp_spectra <- ggplot(rmn_models785_corrected) +
  ...
  scale_y_discrete(
    ...
    labels = rev(c(
      expression(paste(bold("S")[CHOH] ~ DHP)),
      expression(paste(bold("S")[COOH] ~ DHP)),
      expression(paste(bold("G")[COOH] ~ DHP))
    ))
  ) +
  labs(x = expression(paste("Wavenumber [cm"^-1, "]"))) +
