...
pca <- ggplot(gg_pca, aes(x = PC1, y = PC2, fill = cell.type)) +
  ...
  annotate("text",
           x = c(-35, 18, 20, 5),
           y = c(18, 20, -5, -25),
           label = c("Epidermis", "Vascular bundle", "Interfascicular\nfibre", "Pith"),
           family = "Helvetica",
           size = text6,
           hjust = 0) +
  labs(
...
ggplot(data = filter(rmn_data_auc, cell.type == x), aes(x = genotype, y = AUC / 10000)) +
  ...
  theme(
    ...
    axis.title.y = element_blank(),
    axis.ticks.y = element_blank(),
    plot.margin = unit(c(2, 0.85, 2, 0), "mm"),
...
loadings_avg <- ggplot(data = rmn_data_loading, aes(
  ...
  theme(
    legend.position = c(0.8375, 0.935),
    legend.background = element_rect(fill = rgb(1, 1, 1, 0.75),
                                     colour = NA),
    legend.direction = "horizontal",
    legend.key.width = unit(4, "mm"),
...
py <- py %>%
  select(genotype, variable, G, H, `G-OH`, S, CHO) %>%
...
pdf("Raman_figure5.pdf", width = onecol, height = 6)
...
pdf("Raman_figure6.pdf", width = twocol, height = 6)
plot_grid(
  rel_auc,
  rel_lig,
  labels = c("A", "B"),
...
pdf("Raman_figureS3.pdf", width = onecol, height = onecol / 2)
...
s6 <- ggplot(data = rmn_data_ridge) +
  ...
  geom_vline(xintercept = 1603, colour = "black", alpha = 1, size = 0.4, linetype = 2) +
  annotate("text",
           x = 1603,
           y = 0.75,
...
pdf("Raman_figureS6.pdf", width = onecol, height = onecol)
s6
dev.off()
```
