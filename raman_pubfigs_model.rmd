...
spectra_plot <- ggplot(filter(rmn_models, unit %in% c("G", "S"))) +
  ...
  geom_ridgeline(data = tibble(wavenumber = rep(300:1700, 3),
...
auc_boxes <- ggplot(data = rmn_data_all_auc, aes(x = cell.type, y = auc / 1000)) +
  ...
  scale_y_continuous(expand = expand_scale(mult = 0.07),
                     breaks = c(0, 200, 400)) +
  theme(
    axis.text.y = element_blank(),
    axis.title.y = element_blank(),
...
readLines("/home/leonard/Documents/Uni/PhD/IRX/RAMAN/nuoendagula/4cl1＆2/4/4cl1＆2 #4 IF-1.txt") %>%
  str_split("\t") %>%
...
# rmn_data_corrected <- read_csv("rmn_data_corrected.csv")
...
### Table 1 -- identified band and correspondence with literature.
...
heatmap <- ggplot(loadings_long) +
  ...
  theme(axis.text = element_blank(),
        ...
        legend.background = element_rect(fill = rgb(1, 1, 1, 0.75),
                                     colour = NA),
...
py_rmn_corr <- left_join(py, py_rmn) %>%
  ...
  mutate(unit = recode(unit, 
                       ...
                       "S" = "S-R",
                       "CHO" = "R-CHO")) %>%
  mutate(unit = ordered(unit, levels = c("Lignin", "G-R", "G-CHOH", "S-R", "H-R", "R-CHO")),
         units_parsed = factor(unit, labels = c("Lignin",
                                          "G[R]",
                                          "G[CHOH]",
                                          "S[R]",
                                          "H[R]",
...
py_rmn <- rmn_data_corrected %>%
  ...
  summarise(mean.scaled = mean(scaled),
...
py_rmn_corr <- left_join(py, py_rmn) %>%
  mutate(WT = case_when(genotype == "Col-0" ~ "yes",
                        TRUE ~ "no"))
...
ggplot(data = filter(py_rmn_corr, unit == x & wavenumber == y & genotype != z),
       aes(x = mean, y = total.mean.scaled)) +
  geom_rect(data = tidycor,
            xmax = Inf,
            ymax = Inf,
            xmin = -Inf,
            ymin = -Inf,
            aes(fill = estimate,
                x = 0,
                y = 0)) +
  geom_errorbar(aes(ymin = total.mean.scaled - total.sd.scaled,
...
py_plot_lig <- py_plot("Lignin", "1603/4.6 + 1334") +
  labs(x = expression(paste("Lignin * total pyrolysate"^-1)),
...
py_plot_sg <- py_plot("S/G", "1334/1276") +
  labs(x = expression(paste("S"[R], " * G"[R]^{-1})),
    y = expression(paste("1334 cm"^-1,"/ 1276 cm"^-1))) +
  scale_y_continuous(breaks = c(0, 1, 2, 3))

py_plot_cho <- py_plot("GCHO/lig", "1625/lig") +
  labs(x = expression(paste("G"[CHO]," * lignin"^-1)),
    y = expression(paste("1625 cm"^-1,"/ CLB"))) +
  scale_y_continuous(breaks = c(0, 0.5, 1)) +
  # geom_point(data = filter(py_rmn_corr, unit == "GCHO/lig" & wavenumber == "1625/lig" & genotype == "cad4xcad5"),
  #            aes(colour = WT),
  #            size = 1,
  #            alpha = 0.75,
  #            shape = 16) +
  # geom_errorbar(data = filter(py_rmn_corr, unit == "GCHO/lig" & wavenumber == "1625/lig" & genotype == "cad4xcad5"),
  #               aes(ymin = total.mean.scaled - total.sd.scaled,
  #                   ymax = total.mean.scaled + total.sd.scaled),
  #               size = 0.2,
  #               width = 0,
  #               alpha = 0.75) +
  geom_smooth(data = filter(py_rmn_corr, unit == "GCHO/lig" & wavenumber == "1625/lig" & genotype != "cad4xcad5"),
              method = "lm",
              colour = "blue",
                fullrange = T,
                size = 0.4,
...
py_plot_goh <- py_plot("GOH/G", "1664/1603") +
  labs(x = expression(paste("G"^{OH}," * total G-lignin"^-1)),
    y = expression(paste("1664 cm"^-1,"/ 1603 cm"^-1))) 
  # scale_y_continuous(breaks = c(0, 1, 2, 3))

py_plot_snooh <- py_plot("SnoOH", "1334") +
  labs(x = expression(paste("S-lignin "-" S-OH")),
...
#   ungroup() %>%
#   mutate(scaled.z = scale_z(AUC))
# 
# rmn_data_corr <- rmn_data_corr %>%
#   left_join(filter(rmn_data_corr, genotype == "Col-0") %>%
#     ungroup() %>%
#     select(cell.type, scaled.z) %>%
#     group_by(cell.type) %>%
#     summarise(scaled.WT = mean(scaled.z))) %>%
#   mutate(diff = scaled.z - scaled.WT) %>%
#   group_by(genotype, cell.type) %>%
#   summarise(AUC.mean = mean(diff, na.rm = T),
#             AUC.sd = sd(diff, na.rm = T)) %>%
#   pivot_wider(id_cols = genotype, names_from = cell.type, values_from = c(AUC.mean, AUC.sd))
# 
# pdf("AUC_celltypes.pdf")
...
#   geom_rect(aes(xmin = AUC.mean_VB - AUC.sd_VB,
#                 xmax = AUC.mean_VB + AUC.sd_VB,
#                 ymin = AUC.mean_IF - AUC.sd_IF,
#                 ymax = AUC.mean_IF + AUC.sd_IF)) +
#   # geom_point(aes(
#   #   x = VB,
#   #   y = IF,
...
#     plot.margin = unit(c(2, 18, 2, 2), "mm")
#   ) +
#   labs(
...
rmn_data_corr <- left_join(rmn_data_corr, (validated_peaks %>%
                                             mutate(wavenumber = recode(wavenumber, "1603, 1334" = "1603/4.6 + 1334")) %>%
...
rel_auc <- ggplot(data = filter(rmn_data_corr, genotype != "Col-0")) +
  ...
  geom_abline(slope = 1, intercept = 0, linetype = 2, size = 0.2) +
  stat_density2d(data = filter(rmn_data_corr_it, genotype != "Col-0"),
                  aes(x = VB,
                      y = IF,
                      alpha = stat(nlevel),
                      fill = assigned.structure),
...
pdf("Raman_figure6.pdf", width = twocol, height = 5.5)
plot_grid(
  rel_auc,
  rel_lig,
  labels = c("A", "B"),
...
### Figure 7 -- Schematic representation of cell wall metabolic flow 
...
band_contribs <- ggplot(rmn_auc_contrib, aes(x = band, y = proportion)) +
  ...
  labs(x = expression(paste("Raman band [cm"^-1, "]")),
...
auc_corr_lig <- ggplot(rmn_auc, aes(
  ...
  geom_text(
   ...
   geom_text_repel(data = filter(rmn_auc, ((X1603 + X1334) / 1000) > 30 | (AUC / 1000) > 3000),
                    aes(label = genotype),
             size = ggtext_size,
             min.segment.length = 0,
             nudge_x = -500,
...
auc_corr_cellu <- ggplot(rmn_auc, aes(
  ...
  geom_text_repel(data = filter(rmn_auc, ((X1099 + X381) / 1000) > 15 | (AUC / 1000) > 3000),
                    aes(label = genotype),
             size = ggtext_size,
             min.segment.length = 0,
             segment.size = 0.2,
...
auc_corr <- ggplot(rmn_auc, aes(
  ...
  labs(y = "Sum of lignin and cellulose bands",
       x = "Total AUC") +
  geom_text(
    x = 500,
    y = 38,
...
py_plot_hg <- py_plot("H/G", "1383/1276") +
  labs(x = expression(paste("Total H-lignin * total G-lignin"^-1)),
...
py_plot_hs <- py_plot("H/S", "1383/1334", colr = "black") +
  labs(x = expression(paste("Total H-lignin * total S-lignin"^-1)),
...
coef_table <- kable(rmn_model_scoeff,
  ...
  add_header_above(c(" " = 1, "Relative AUC\\\\textsuperscript{a}" = 2,
                     "Signal\\\\textsubscript{fluo} * Signal\\\\textsubscript{Raman}\\\\textsuperscript{-1}" = 2), 
                   escape = FALSE) %>%
...
rmn_model_supp <- rmn_models %>%
  filter(state == "mono") %>%
  mutate(filename = recode(filename,
                           ...
                           "mono-s-cho" = "S-CHO",
                           "mono-s-oh" = "S-OH"),
         filename = ordered(filename, levels = rev(c("G-OH",
                                                     "Gsat-OH",
                                                     "G-CHO",
                                                     "G-COOH",
                                                     "5H-COOH",
...
supp_spectra <- ggplot(rmn_model_supp) +
  ...
  scale_y_discrete(expand = expand_scale(mult = c(0.15, 0.1)),
                   labels = rev(c(
    expression(paste("G"[CHOH])),
    expression(paste("G"[sat-CHOH])),
    expression(paste("G"[CHO])),
...
supp_spectra <- ggplot(rmn_models785_corrected) +
  ...
  scale_y_discrete(expand = expand_scale(mult = c(0.2, 0.1)),
                   labels = rev(c(
...
auc_data <- rmn_data_corrected %>%
  ...
  left_join(cml_data_length)

broom::glance(lm(AUC ~ length, data = auc_data))

pdf("AUC_to_width.pdf")
ggplot(auc_data, aes(x = AUC, y = length)) +
  geom_point(aes(x = AUC, y = length)) +
  geom_smooth(method = "lm") +
...
rmn_var <- rmn_data_corrected %>%
  select(genotype, cell.type, replicate, technical, corrected.intensity, scaled, scaled_max) %>%
  pivot_longer(cols = c("corrected.intensity", "scaled", "scaled_max"),
               names_to = "scaling",
               values_to = "value") %>%
  group_by(genotype, cell.type, scaling) %>%
