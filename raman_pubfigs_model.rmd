...
rmn_data_corrected <- rmn_data_corrected %>%
  ...
  mutate(
    ...
    scaled_max = case_when(wavenumber == 1625 ~ corrected.intensity / corrected.intensity[wavenumber == 1603],
                           TRUE ~ corrected.intensity / (
...
rmn_data_ridge_full_var <- rmn_data_corrected %>%
  ungroup() %>%
  mutate(
    genotype = ordered(genotype, levels = c(
      "Col-0",
...
#### Figure 4 A --  Full PCA and PCA loading supplement
...
top <- plot_grid(
  fviz_pca_var(pca_result,
               select.var = list(name = c("1603", "1119", "1257", "1149")),
               repel = TRUE,
               title = "",
...
#### Figure 4 A II -- Split PCA and PCA loading supplement
...
pca <- ggplot(gg_pca, aes(x = PC1, y = PC2, colour = genotype, shape = cell.type)) + 
  geom_hline(yintercept = 0, linetype = 1) +
  geom_vline(xintercept = 0, linetype = 1) +
  # stat_ellipse(geom = "polygon", alpha = 0.5, type = "t", level = 0.4) +
  # stat_ellipse(aes(fill = NULL), colour = "black", linetype = 2) +
...
bottom_PC1 <- ggplot(pca_load, aes(x = wavenumber, y = PC1)) +
  geom_line() +
  labs(title = "PC1") +
  # geom_text_repel(data = filter(pca_load, PC1 > 0.055),
  #           aes(label = wavenumber)) +
  # geom_text_repel(data = filter(pca_load, PC1 < - 0.065),
  #           aes(label = wavenumber)) +
  theme_leo()
...
bottom_PC2 <- ggplot(pca_load, aes(x = wavenumber, y = PC2)) +
  ...
  # geom_text_repel(data = filter(pca_load, PC2 > 0.08),
  #           aes(label = wavenumber)) +
  # geom_text_repel(data = filter(pca_load, PC2 < - 0.09),
  #           aes(label = wavenumber)) +
  theme_leo()
...
#### Figure Sx -- correlations between peaks

This shows the strong correlation between the cellulose peaks and the "H"-peak. I believe this is the only point we can 

```{r, fig.showtext = TRUE, out.width = '100%', warning = FALSE, message = FALSE, echo = FALSE, fig.asp = 1.5}
...
chart.Correlation(py_rmn, histogram=TRUE, pch=21)
```
...
py <- py %>%
  select(genotype, variable, Lignin, G, H, `G-OH`, S, Aldehydes) %>%
...
ggplot(data = py_rmn_corr) +
  ...
  facet_grid(unit ~ reorder(wavenumber, as.numeric(wavenumber))) +
  coord_fixed()
# dev.off()
```
...
py <- py %>%
  select(genotype, variable, Lignin, `S/G`, `H/G`, `H/S`) %>%
  pivot_longer(-c(genotype, variable), names_to = "unit", values_to = "value") 

py_rmn <- rmn_data_avg %>%
  filter(wavenumber %in% selected_peaks$wavenumber) %>%
  group_by(cell.type, genotype, wavenumber) %>%
  pivot_wider(id_cols = c(genotype, cell.type), names_from = wavenumber,
              values_from = mean.scaled) %>%
  transmute("1603/4.6 + 1334" = (`1603` / 4.6) + `1334`,
         "1334/1603" = `1334` / `1603`,
         "1334/1276" = `1334` / `1276`,
         "1383/1276" = `1383` / `1276`,
         "522/1276" = `522` / `1276`,
...
**NOTE:** 1625 is relative to total G-units.
...
py <- py %>%
  ...
  mutate(`G-CHO` = `G-CHO` / G * Lignin) %>% # is the ratio between normalised and untransformed values identical?
  select(-c(3:28, Lignin, Sinapaldehyde, Syringaldehyde, Vanillin)) %>%
  pivot_longer(-c(genotype, variable), names_to = "unit", values_to = "value") 

py_rmn <- rmn_data_avg %>%
  filter(wavenumber %in% c(1664, 1625, 1603, 1383, 1334, 1276)) %>%
  group_by(genotype, wavenumber) %>%
...
ggplot(data = py_rmn_corr) +
  ...
  scale_fill_distiller(palette = "RdYlBu", 
                       ...
                       limits = c(-1,1)) +
  labs(
    x = expression(paste("Pyrolysate * total lignin pyrolysate"^"-1")),
    y = expression(paste("Raman average (IF:VB 1:1) * total lignin peaks"^"-1"))
  ) +
  theme_leo() +
  theme(
    legend.position = "bottom",
...
kable(validated_peaks,
  ...
  add_header_above(c(" " = 5, "Total pyrolysate" = 2, "Total lignin" = 2, "G-lignin" = 2))
```
...
rmn_data_corr <- rmn_data_corrected %>%
  filter(wavenumber %in% c(validated_peaks$wavenumber) & 
           wavenumber != 1276) %>%
  # group_by(genotype, cell.type, replicate, technical) %>%
...
ggplot(data = filter(rmn_data_corr, genotype != "Col-0")) +
  ...
  scale_fill_manual(values = c("G-CHO" = "#5DA5DA", 
                               "H-units" = "#FAA43A",
                               "G-OH" = "#60BD68",
                               "S-units" = "#F17CB0",
                               "Total lignin" = "#B2912F")) +
  scale_colour_manual(values = c("G-CHO" = "#5DA5DA", 
                               "H-units" = "#FAA43A",
                               "G-OH" = "#60BD68",
                               "S-units" = "#F17CB0",
                               "Total lignin" = "#B2912F")) +
...
rmn_data_corr <- rmn_data_corrected %>%
  filter(wavenumber %in% c(validated_peaks$wavenumber, 381, 1099) &
    wavenumber != 1625 & wavenumber != 1383 & wavenumber != 1276) %>%
  mutate(wavenumber = as.character(wavenumber)) %>%
  ungroup() %>%
  pivot_wider(id_cols = c(genotype:technical),
              names_from = wavenumber,
...
rmn_data_corr <- left_join(rmn_data_corr, (validated_peaks %>%
  rbind(list(13, "381", "AUC", "Cellulose", "Cellulose", 0, 0, 0, 0, 0, 0)) %>%
  rbind(list(10, "1099", "AUC", "Cellulose", "Cellulose", 0, 0, 0, 0, 0, 0))))
...
Main take-aways from the analysis using the composite "total lignin" measure:
...
* In turn, *fah1* is specifically increased in G-OH, while total lignin remains constant.
* *omt1* is decreased in total lignin and S-units, with unchanged levels of G-OH.
* While this more sophisticated model to quantify total lignin does give better results for the Py-GC/MS correlations of Total lignin and G-OH, it renders the correlation with aldehydes insignificant. This mainly poses a problem for the *irx*-paper, in which the Raman aldehyde measure is a cruscial part of the models.

### Figure 6 -- Explanation of AUC by total lignin and cellulose
...
ggplot(rmn_auc_contrib, aes(x = band, y = proportion)) +
  geom_quasirandom(aes(fill = band),
                   shape = 21,
                   alpha = 0.5,
                   stroke = 0.1) +
...
rmn_auc <- rmn_data_corrected %>%
  ...
  pivot_wider(id_cols = c(genotype, cell.type, replicate, technical, AUC),
              names_from = wavenumber,
...
ggplot(rmn_auc, aes(x = AUC,
                 y = X1603 + X1334 + X1099 + X381)) +
...
rmn_auc_lm <- broom::glance(lm(AUC ~ X1603 + X1334 + X1099, data = rmn_auc))
rmn_auc_lm_full <- broom::tidy(lm(AUC ~ X1603 + X1334 + X1099, data = rmn_auc))
