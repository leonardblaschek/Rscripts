...
rmn_data_corrected <- rmn_data_corrected %>%
  ...
  mutate(
    AUC = MESS::auc(wavenumber, intensity),
    AUC_1603 = MESS::auc(wavenumber, intensity, from = 1580, to = 1640),
    AUC_1334 = MESS::auc(wavenumber, intensity, from = 1320, to = 1358),
    AUC_1099 = MESS::auc(wavenumber, intensity, from = 1081, to = 1111),
    AUC_381 = MESS::auc(wavenumber, intensity, from = 376, to = 398),
...
py <- py %>%
  select(genotype, variable, G, H, `G-OH`, Lignin, S, `S-OH`, CHO) %>%
...
py_rmn_corr <- left_join(py, py_rmn) %>%
  ungroup() %>%
  mutate(unit = ordered(unit, levels = c("Lignin", "G", "G-OH", "S", "S-OH", "H", "CHO")))
...
total_corr <- ggplot(data = py_rmn_corr) +
  ...
  facet_grid(reorder(wavenumber, as.numeric(wavenumber)) ~ unit) 
...
py_rmn <- rmn_data_corrected %>%
  ...
  transmute(
    "1603/4.6 + 1334" = (`1603` / 4.6) + `1334`,
    "1334/1603" = `1334` / `1603`,
    "1664/1603" = `1664` / `1603`,
    "1334/1276" = `1334` / `1276`,
    "1625/1603" = `1625` / `1603`,
    "1625/1664" = `1625` / `1664`,
    "1625/lig" = `1625` / ((`1603` / 4.6) + `1334`),
    "1625" = `1625`,
    "1334" = `1334`
  ) %>%
...
broom::glance(lm(mean ~ total.mean.scaled, data = filter(py_rmn_corr, unit == "GCHO/lig" & 
                                                           wavenumber == "1625/lig" &
                                                           genotype != "cad4xcad5")))
broom::glance(lm(mean ~ total.mean.scaled, data = filter(py_rmn_corr, unit == "GCHO" & 
                                                           wavenumber == "1625" &
                                                           genotype != "cad4xcad5")))
...
py_plot_lig <- py_plot("Lignin", "1603/4.6 + 1334") +
  labs(x = expression(paste("Total lignin * total pyrolysate"^-1)),
    y = expression(paste("CLB [AU * AUC"^-1,"]"))) +
  scale_y_continuous(breaks = c(0, 0.0025, 0.005), labels = c("0", "0.025", "0.005"))

py_plot_sg <- py_plot("S/G", "1334/1276") +
  labs(x = expression(paste("Total S-lignin * total G-lignin"^-1)),
    y = expression(paste("1334 cm"^-1,"/ 1276 cm"^-1))) +
  scale_y_continuous(breaks = c(0, 1, 2, 3))

py_plot_cho <- py_plot("GCHO/lig", "1625/lig", "cad4xcad5") +
  labs(x = expression(paste("Total G-aldehydes * total lignin"^-1)),
  ...
  geom_errorbar(data = filter(py_rmn_corr, unit == "GCHO/lig" & wavenumber == "1625/lig" & genotype == "cad4xcad5"),
                ...
                width = 0,
                alpha = 0.75) +
  geom_smooth(data = filter(py_rmn_corr, unit == "GCHO/lig" & wavenumber == "1625/lig"),
              method = "lm",
              colour = "black",
                fullrange = T,
                size = 0.4,
...
pdf("Raman_figure5.pdf", width = twocol, height = 5)
...
pdf("Raman_figureS3.pdf", width = onehalfcol, height = onehalfcol / 2)
...
supp_spectra <- ggplot(rmn_model_supp) +
  # geom_text_repel(
  #   data = tibble(
  #     wavenumber = rep(c(1656, 1620, 1600, 1335, 1276), 2),
  #     unit = rep(c("G", "S"), each = 5, len = 10),
  #     gamma = rep("COOH", 10),
  #     scaled = rep(1, 10)
  #   ),
