...
writexl::write_xlsx(kinetics_nested, "TE_activity_slopes.xlsx")
```
...
plate_files <-
  list.files(
    path = paste0(datapath, "PhD/Laccase_activity/2022-05_STS_activity/csv_files/plates/"),
    pattern = "*.csv$",
    recursive = FALSE,
    full.names = TRUE
  )
...
STS_plates <- map_dfr(plate_files, read_plus)
```
...
spec_files <-
  list.files(
    path = paste0(datapath, "PhD/Laccase_activity/2022-05_STS_activity/csv_files/absorbance/"),
    pattern = "*.csv$",
    recursive = FALSE,
    full.names = TRUE
  )
...
STS_spec <- map_dfr(spec_files, read_vroom) %>%
  pivot_longer(c(`220`:`1000`), names_to = "wavelength", values_to = "measurement") %>%
  left_join(STS_plates, by = c("date", "started", "Well")) %>%
...
pdf("STS_kinetics.pdf", height = twocol, width = twocol)
ggplot(
  STS_kinetics,
  aes(
    x = time,
  ...
  geom_smooth(
    data = filter(STS_kinetics, time %in% c(lm_start:lm_stop)),
    aes(group = interaction(replicate, sample, pH)),
    method = "lm",
    size = 0.5,
    se = F
...
STS_kinetics_nested <- STS_kinetics %>%
  filter(sample != "neg. ctrl.") %>% 
  filter(time >= lm_start & time <= lm_stop) %>%
  select(-absorbance, -adjusted_absorbance) %>%
  nest(data = c(time, corrected_absorbance)) %>%
...
STS_kinetic_max <- STS_kinetics_nested %>%
  group_by(pH, sample, substrate, day) %>%
  summarise(scaled_estimate = mean(scaled_estimate)) %>%
  group_by(pH, sample, substrate) %>%
  mutate(
...
pdf("STS_activity_slopes.pdf", width = onecol, height = onecol)
slope_grid
dev.off()
```

### STS activation pilot
...
STS_spec <- map_dfr(spec_files, read_vroom) %>%
  pivot_longer(c(`220`:`1000`), names_to = "wavelength", values_to = "measurement") %>%
  left_join(STS_plates, by = c("date", "started", "Well")) %>%
  mutate(treatment = case_when(sample == "neg. ctrl." ~ "control",
                               TRUE ~ treatment)) %>% 
  filter(!is.na(treatment)) %>% 
...
STS_data <- STS_spec %>%
  ...
  group_by(date, replicate, substrate, pH, wavelength, time) %>%
  mutate("blank" = absorbance[sample == "neg. ctrl."]) %>%
  group_by(date, sample, substrate, pH, replicate, day, treatment) %>%
  mutate(
    density = mean(absorbance[wavelength == 600 & `Cycle #` %in% c(1, 2)]),
    adjusted_absorbance = case_when(
      sample == "neg. ctrl." ~ absorbance,
...
STS_kinetics <- STS_data %>%
  ...
  left_join(lm_bounds) %>%
  group_by(
    date, sample, substrate, pH, replicate, day, time, treatment, lm_start, lm_stop, density
  ) %>%
  mutate(corrected_absorbance = zeroed_absorbance - zeroed_absorbance[wavelength == abs_min]) %>%
  summarise(
    corrected_absorbance = corrected_absorbance[wavelength == abs_max],
...
pdf("STS_activation_kinetics.pdf", height = twocol, width = twocol)
ggplot(
  STS_kinetics,
  aes(
    x = time,
...
#### Check boiled absorbance

```{r}
test <- STS_data %>% 
  filter(treatment %in% c("control", "boiled") & 
           date %in% c(as_date("2022-05-13")) & 
           substrate == "ABTS")
...
STS_kinetics_nested <- STS_kinetics %>%
  ...
  mutate(treatment_length = case_when(date == as_date("2022-05-13") ~ "1 h",
                                      date == as_date("2022-05-17") ~ "24 h"))
...
slope_grid <- ggplot(
  ...
  labs(y = "Substrate oxidation rate") +
  theme_leo() +
  theme(legend.position = "bottom") +
  facet_grid(pH ~ substrate)
