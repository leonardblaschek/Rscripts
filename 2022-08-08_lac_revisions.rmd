...
# High resolution Wiesner images
...
As discussed on Monday, in addition to the stem base angle (**A**) I've estimated a new metric I call *straightness*, which is the height of the bounding box of the main stem divided by the total length of the main stem. Consequently, straightness is $\approx$ 1 for completely straight plants, and goes toward 0 for plants that have fallen over, are curled, or both (**C**). Unfortunately, while that metric was able to pinpoint more unstable plants than the stem base angle, they all were of different genotypes. In the end, straightness is not actually significantly different in any mutant. I then related both the stem base angle and the straightness to stem length, by separating each genotype into two groups along the respective median stem length (**B**, **D**). However, as you see below, that didn't add anything either. The only consequence of doing so is that I lost the statistical power to detect the differences that are there overall.

```{r, echo=FALSE, fig.align='center'}
knitr::include_graphics("lodging.pdf")
```

I also tried a mixed model, predicting the straightness or stem base angle from the plant height and the genotype. The resulting model is significant for the stem base angle (mirroring the significant differences between genotypes in **A** above), but the R\textsuperscript{2} is only 0.13, so I don't think we should include that.

This has convinced me that the issue here is not the metric but the biological variability of this "lodging" phenotype. At this point, I don't see a better alternative than figure S2 as I've sent it to you earlier:
...
```{r, echo=FALSE, fig.align='center'}
knitr::include_graphics("maule_plot_OD.pdf")
```

# Correlate Mäule hue to S/G

Played around with it a little, best result see below. *Best* here means good model that is also easy to interpret. If I take the log of the Mäule hue as well, the R\textsuperscript{2} is 0.01 higher. But then I couldn't add the visual scale to ease interpretation, so I stuck with \texttt{hue\textasciitilde ln(S/G)}.

```{r, echo=FALSE, fig.align='center'}
knitr::include_graphics("maule_SG.pdf")
```

# Stem thickness/height ratio

Here's the relevant data. Summarised very concisely, loss of *LAC*s affects only axial, not radial growth. Consequently, I don't think the thickness/height ratio adds anything that isn't shown in the stem thickness and stem number plots. Those two we could include, since they also suggest an answer to the question whether tissue organisation is significantly changed: no it isn't.

```{r, echo=FALSE, fig.align='center'}
knitr::include_graphics("thickness.pdf")
```
...
# *Optional*: Tissue composition

> *Additionally, measurements of the relative vascular tissues in setm section for each genotype to define (%tage of IF, %tage of VB and within %tage of MX, XF and PX?)*
